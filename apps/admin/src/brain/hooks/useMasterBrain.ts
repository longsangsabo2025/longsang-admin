...
export function useMasterBrainQuery() {
  ...
  >({
    mutationFn: ({ query, sessionId, options }) =>
      brainAPI.masterBrainQuery(query, sessionId, options),
    onError: (error) => {
...
export function useCreateMasterSession() {
  ...
  return useMutation<string, Error, { sessionName: string; domainIds: string[]; options?: any }>({
...
export function useEndMasterSession() {
  ...
  return useMutation<void, Error, { sessionId: string; rating?: number; feedback?: string }>({
    mutationFn: ({ sessionId, rating, feedback }) =>
      brainAPI.endMasterSession(sessionId, rating, feedback),
