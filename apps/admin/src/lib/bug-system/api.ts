 ...
 * @param mode - 'scan' (default, no file changes) | 'fix' (modify files, may trigger HMR) | 'analyze' (analyze patterns) | 'clear' (clear all db errors)
 */
export async function triggerAutoFix(
  mode: 'scan' | 'fix' | 'analyze' | 'clear' = 'scan'
  ...
  try {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/bug-system/auto-fix?mode=${mode}`, {
        method: 'POST',
        headers: {
