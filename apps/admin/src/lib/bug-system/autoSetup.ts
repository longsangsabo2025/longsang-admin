...
export function setupGlobalErrorHandlers(): void {
  ...
  console.error = (...args: any[]) => {
    ...
    // Expanded loop prevention - skip internal system errors
    const skipPatterns = [
      '[ErrorHandler]',
      '[AutoSetup]',
    ...
    if (skipPatterns.some(p => errorString.includes(p))) {
      return;
    }

    // Capture console.error
    const error = args[0] instanceof Error ? args[0] : new Error(errorString.slice(0, 500));
  ...
  window.fetch = async (...args: Parameters<typeof fetch>) => {
    ...
    try {
      ...
      const error =
        networkError instanceof Error ? networkError : new Error('Network request failed');
