...
class ErrorHandler {
  ...
  async capture(error: Error | unknown, context: ErrorContext = {}): Promise<string | null> {
    try {
      ...
      // Skip for bug-system related errors and backend API errors to prevent noise
      const isBugSystemError =
        normalizedError.message.includes('bug-system') ||
        normalizedError.message.includes('ai-analyze') ||
        normalizedError.message.includes('429') ||
        normalizedError.message.includes('Too Many Requests') ||
        normalizedError.message.includes('404') ||
        normalizedError.message.includes('Not Found') ||
        normalizedError.message.includes('500') ||
        normalizedError.message.includes('Internal Server Error') ||
        normalizedError.message.includes('/api/brain/') ||
        context.component === 'AIFixService' ||
        context.component === 'Network' ||
        context.action === 'ai-analyze';

      if (errorLogId && !isBugSystemError) {
