...
if (isValidData(response)) {
  ...
  private async saveSuggestions(errorId: string, suggestions: FixSuggestion[]): Promise<void> {
    try {
      ...
      const { error } = await supabase.from('ai_fix_suggestions').insert(records);
      if (error) {
        // Table may not exist - silently fail
        logger.debug('ai_fix_suggestions table not accessible', error as Error, 'AIFixService');
      }
    } catch (error) {
  ...
  async getSuggestions(errorId: string): Promise<FixSuggestion[]> {
    try {
      ...
      logger.debug('Failed to get suggestions', error as Error, 'AIFixService');
      return [];
    }
  }
