                ...
                <CardHeader className="pb-3">
                    ...
                    <div>
                      <CardTitle className="text-base flex items-center gap-2">
                        ...
                        {activeRun?.input?.topic && (
                          <Badge variant="secondary" className="text-[10px] font-normal max-w-[260px] truncate">
                            {activeRun.input.topic}
                          </Badge>
                        )}
                        {!activeRunId && topic.trim() && (
                          <Badge variant="outline" className="text-[10px] font-normal text-green-600 border-green-500">
                            New Topic
                          </Badge>
                        )}
                      </CardTitle>
                  ...
                  {channelRuns.length > 0 && (
                    <div className="flex items-center gap-1.5 flex-wrap pt-2">
                      ...
                      {channelRuns.slice(0, 8).map((run) => {
                        const isActive = activeRunId === run.id;
                        const isRunning = runningRunIds.has(run.id);
                        const label = run.input?.topic?.slice(0, 28) || run.input?.transcript?.slice(0, 20) || run.id.slice(0, 8);
                        return (
                          <Button
                            key={run.id}
                            ...
                            className={cn(
                              ...
                              isRunning && !isActive && 'border-blue-400 text-blue-600 animate-pulse',
                            )}
...
function VoiceTabContent({
  ...
  useEffect(() => {
    if (voiceoverConfig?.engine !== 'elevenlabs' || !voiceoverConfig?.voice) {
    ...
    const cacheKey = `voice-preview-vi-elevenlabs__${voiceoverConfig.voice}`;
    try {
    ...
    fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(voiceoverConfig.voice)}`, {
      method: 'POST',
