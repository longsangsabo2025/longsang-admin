...
export const ZaloCampaignDashboard = () => {
  ...
  const prepareMessage = (template: MessageTemplate | null, customerName: string, voucher: Voucher | undefined) => {
    let message = template?.content || 'Chào {name}! Bạn nhận được voucher {voucher_value} phút miễn phí tại SABO. Mã: {voucher_code}';
  ...
  const handleStartCampaign = async () => {
    ...
    for (let i = 0; i < pendingTargets.length; i++) {
      try {
        ...
        await campaignService.updateTargetStatus(pendingTargets[i].id, 'failed', undefined, 'Lỗi hệ thống');
      }
          ...
          {selectedCampaign ? (
            ...
            <Card className="flex items-center justify-center h-[400px]">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">
                  Bắt đầu warm-up khách hàng cold của bạn
                </p>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="bg-muted/50 rounded-lg p-4 space-y-2">
              ...
              <ul className="text-sm space-y-1 text-muted-foreground">
                <li>• Voucher: <strong>{warmUpConfig.voucherValue} phút FREE</strong></li>
                <li>• Hiệu lực: <strong>{warmUpConfig.voucherValidDays} ngày</strong></li>
                <li>• Đối tượng: <strong>Khách hàng Cold</strong></li>
                <li>• Nhắc nhở: <strong>{warmUpConfig.autoReminder ? `Trước ${warmUpConfig.reminderDays} ngày` : 'Không'}</strong></li>
              </ul>
            </div>
          </div>
