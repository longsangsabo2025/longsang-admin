...
export const MVP_AGENTS: MVPAgent[] = [
  {
    ...
    description:
      'AI Agent thông minh phân tích thông tin contact, đánh giá chất lượng lead và đề xuất hành động tiếp theo. Giúp team sales tập trung vào leads tiềm năng cao nhất.',
    icon: '🎯',
    category: 'sales',
...
Respond in JSON format with clear, actionable insights.`,
...

  {
    ...
    description:
      'AI Content Writer chuyên nghiệp tạo blog posts chất lượng cao với SEO optimization, internal linking, và CTA hấp dẫn. Giảm 90% thời gian viết content.',
    icon: '✍️',
    category: 'content',
...
Output Format: JSON with title, meta_description, content, seo_metrics, suggestions`,
...

  {
    ...
    description:
      'AI Agent viết và lên lịch email follow-up theo context, giúp tăng response rate lên 3x. Tự động personalize dựa trên previous conversation và recipient profile.',
    icon: '📧',
    category: 'sales',
...
Output Format: JSON with subject, body, send_time_suggestion, follow_up_plan`,
...

  {
    ...
    description:
      'AI Agent tạo content cho Facebook, LinkedIn, Twitter, Instagram và TikTok từ 1 brief duy nhất. Tự động optimize cho từng platform với hashtags và best posting time.',
    icon: '📱',
    category: 'marketing',
...
Output Format: JSON with posts for each platform, hashtags, posting times, engagement tips`,
...

  {
    ...
    description:
      'AI Agent phân tích spreadsheets, tìm patterns, detect anomalies và tạo executive summary với visualizations. Transform raw data thành actionable insights.',
    icon: '📊',
    category: 'data',
...
export const getFeaturedAgents = (limit: number = 3): MVPAgent[] => {
  return [...MVP_AGENTS].sort((a, b) => b.rating.score - a.rating.score).slice(0, limit);
};
