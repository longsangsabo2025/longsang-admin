...
import { useRegistryAgents, useAgentStats } from '@/hooks/use-agent-company';
...
export function MorningBriefing() {
  ...
  // Real data from agent_registry (45 agents)
  const { data: registryAgents = [] } = useRegistryAgents({});
  const { data: registryStats } = useAgentStats();

  // Fallback to mock data if no real data
  const [items, setItems] = useState<BriefingItem[]>(mockBriefingItems);

  // Map real registry agents to AgentStatus format (top 8 active agents)
  const agents: AgentStatus[] = registryAgents.length > 0
    ? registryAgents
        ...
        .map((a) => ({
          ...
          name: a.name,
          icon: <Zap className="h-4 w-4" />,
          status: a.status === 'busy' ? 'active' as const : a.status as 'active' | 'idle' | 'error',
          lastAction: a.description || a.role,
          completedTasks: a.total_executions,
          pendingTasks: 0,
        }))
    : mockAgentStatuses;
  ...
  const handleApprove = (itemId: string) => {
    ...
    toast({ title: 'Approved', description: item?.title || 'Item approved' });
  };

  const handleReject = (itemId: string) => {
    const item = items.find((i) => i.id === itemId);
    setItems((prev) => prev.filter((i) => i.id !== itemId));
    toast({ title: 'Rejected', description: item?.title || 'Item rejected', variant: 'destructive' });
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-500">
                    {registryStats?.active ?? agents.filter((a) => a.status === 'active').length}/{registryStats?.total ?? agents.length}
                  </p>
                </div>
