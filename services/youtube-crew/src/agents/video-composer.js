...
export class VideoComposerAgent extends BaseAgent {
  ...
  /**
   * Verify output video duration matches expected audio duration.
   * Logs warning if drift exceeds threshold (default 0.5s).
   */
  verifyDuration(videoPath, expectedDuration, thresholdSec = 0.5) {
    try {
      const absPath = resolve(videoPath).replace(/\\/g, '/');
      const result = execSync(
        `"${this.ffprobe}" -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${absPath}"`,
        { encoding: 'utf-8', timeout: 30000 }
      ).trim();
      ...
      if (drift > thresholdSec) {
        this.log(`⚠️ A/V drift detected: expected ${expectedDuration.toFixed(1)}s, got ${actualDuration.toFixed(1)}s (drift: ${drift.toFixed(2)}s)`, 'warn');
      }
      return { actualDuration, expectedDuration, drift };
    } catch (err) {
      this.log(`⚠️ Could not verify video duration: ${err.message}`, 'warn');
      return null;
    }
  }
...
EXAMPLE: A single glowing hourglass suspended in complete darkness, golden sand flowing creating a luminous trail, dramatic rim lighting from behind casting warm amber glow, deep shadow everywhere else, ultra sharp, cinematic, photorealistic, hyper detailed`;
  ...
  async execute(task, context = {}) {
    ...
    // Post-render A/V sync verification
    this.verifyDuration(videoPath, duration);
