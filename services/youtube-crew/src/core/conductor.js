...
export class Conductor {
  ...
  /**
   * Check if a pipeline with the same input is already running.
   * @returns {{ duplicate: boolean, pipelineId?: string }}
   */
  checkDuplicateRun(pipelineName, input) {
    const inputKey = input.topic || input.videoUrl || '';
    if (!inputKey) return { duplicate: false };
    for (const r of this.activeRuns.values()) {
      if (r.pipelineName === pipelineName && r.status === 'running' && r.inputKey === inputKey) {
        return { duplicate: true, pipelineId: r.pipelineId };
      }
    }
  ...
  async executePipeline(pipelineName, input = {}, opts = {}) {
    ...
    const run = {
      ...
      pipelineId,
      pipelineName,
      inputKey: input.topic || input.videoUrl || '',
      startTime,
      status: 'running',
      stageResults: {},
      errors: [],
