    ...
    <div className="space-y-6">
      <Tabs defaultValue="podcasts" className="space-y-4">
        ...
        <TabsContent value="podcasts">
          <PodcastManager domains={domains || []} selectedDomainId={selectedDomainId} />
        </TabsContent>

        {/* Audio Upload */}
        <TabsContent value="upload">
          <AudioUploader domains={domains || []} selectedDomainId={selectedDomainId} />
        </TabsContent>

        {/* Voice Notes */}
        <TabsContent value="voice">
          <VoiceNoteRecorder domains={domains || []} selectedDomainId={selectedDomainId} />
        </TabsContent>
              ...
              {subscriptions.map((sub) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{sub.episodeCount} episodes</p>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {['Huberman Lab', 'Lex Fridman', 'Tim Ferriss', 'Naval'].map((name) => (
              <Button
                ...
                onClick={() =>
                  setNewFeedUrl(`https://${name.toLowerCase().replace(' ', '')}.com/feed`)
                }
              >
...
function AudioUploader({
  ...
  const transcribeFiles = async () => {
    ...
    for (const file of files.filter((f) => f.status === 'pending')) {
      setFiles((prev) =>
        prev.map((f) => (f.id === file.id ? { ...f, status: 'transcribing' as const } : f))
      );
      ...
      try {
        ...
        setFiles((prev) =>
          prev.map((f) => (f.id === file.id ? { ...f, status: 'done' as const, transcript } : f))
        );
      } catch (err) {
        setFiles((prev) =>
          prev.map((f) =>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Upload audio files để transcribe bằng Whisper AI</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Domain Selection */}
        <div className="space-y-2">
            ...
            {files.map((file) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {file.status === 'transcribing' && <Progress value={50} className="h-1 mt-1" />}
                </div>
                {file.status === 'transcribing' && (
          ...
          >
            {processing ? (
              ...
              <>
                ...
                Transcribe {files.filter((f) => f.status === 'pending').length} files
              </>
            )}
          </Button>
        )}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Ghi âm voice notes hoặc meeting và tự động transcribe</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Domain Selection */}
        <div className="space-y-2">
...
function AudioHistory() {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'podcast':
        return <Podcast className="h-5 w-5 text-purple-500" />;
      case 'voice':
