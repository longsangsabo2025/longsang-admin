...
import { Brain, Check, FolderOpen, MapPin, Plus, User } from 'lucide-react';
...
export function ImageBrainDialogs({
  ...
  return (
    <>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  setNewCharacter((prev) => ({ ...prev, isOwner: !!checked }))
                }
              />
      ...
      <Dialog open={showCreateCollectionDialog} onOpenChange={setShowCreateCollectionDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Nhóm các ảnh liên quan lại với nhau để dễ quản lý</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
      ...
      {/* Add Images to Character Dialog */}
      <Dialog
        open={!!addImageToCharacterId}
        onOpenChange={(open) => !open && setAddImageToCharacterId(null)}
      >
        <DialogContent className="max-w-3xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <User className="w-5 h-5" />
              Thêm ảnh cho nhân vật:{' '}
              {normalizedCharacters.find((c) => c.id === addImageToCharacterId)?.name}
            </DialogTitle>
            <DialogDescription>Chọn ảnh từ thư viện để liên kết với nhân vật này</DialogDescription>
          </DialogHeader>
      ...
      {/* Add Images to Location Dialog */}
      <Dialog
        open={!!addImageToLocationId}
        onOpenChange={(open) => !open && setAddImageToLocationId(null)}
      >
        <DialogContent className="max-w-3xl max-h-[80vh]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {normalizedLocations.find((l) => l.id === addImageToLocationId)?.name}
            </DialogTitle>
            <DialogDescription>Chọn ảnh từ thư viện để liên kết với địa điểm này</DialogDescription>
          </DialogHeader>
          <ScrollArea className="h-[400px]">
            <div className="grid grid-cols-4 gap-2 p-2">
              {normalizedImages.map((img) => {
