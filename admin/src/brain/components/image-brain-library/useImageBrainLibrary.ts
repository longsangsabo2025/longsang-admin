...
import type { ImageCategory, ImageMemoryItem } from '@/brain/types/image-memory.types';
import type { UseImageBrainLibraryReturn } from './types';
import { normalizeCharacter, normalizeImage, normalizeLocation } from './utils';
...
export function useImageBrainLibraryHook({
  ...
  // State
  const [activeTab, setActiveTab] = useState<'all' | 'characters' | 'locations' | 'collections'>(
    'all'
  );
  ...
  const handleUpdateImage = useCallback(
    (imageId: string, updates: Partial<ImageMemoryItem>) => {
  ...
  const handleAddImagesToCharacter = useCallback(
    (imageIds: string[]) => {
  ...
  const handleAddImagesToLocation = useCallback(
    (imageIds: string[]) => {
