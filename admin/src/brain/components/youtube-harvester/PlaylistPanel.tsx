...
import { AlertCircle, Brain, ListVideo, Loader2, Search } from 'lucide-react';
...
export function PlaylistPanel() {
  ...
  const toggleVideo = (videoId: string) => {
    setVideos(videos.map((v) => (v.id === videoId ? { ...v, selected: !v.selected } : v)));
  };
    ...
    for (let i = 0; i < selectedVideos.length; i++) {
      ...
      try {
        ...
        const transcriptRes = await fetch(
          `${API_BASE}/api/brain/youtube/transcript/${video.id}?lang=vi`
        );
        ...
        setVideos((prev) => prev.map((v) => (v.id === video.id ? { ...v, selected: false } : v)));
      } catch (err) {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>Import toàn bộ playlist YouTube với xử lý hàng loạt</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {videos.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <Label>
                Videos ({selectedCount}/{videos.length} đã chọn)
              </Label>
              <div className="flex gap-2">
