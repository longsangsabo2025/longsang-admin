...
import { AlertCircle, ExternalLink, ListVideo, Loader2, Plus, Search, Users } from 'lucide-react';
...
export function ChannelPanel() {
  ...
  const fetchChannelInfo = async () => {
    ...
    if (!extracted) {
      setError(
        'URL không hợp lệ. Hỗ trợ: youtube.com/channel/UC..., youtube.com/@handle, hoặc @handle'
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/brain/youtube/channel/${extracted.value}?type=${extracted.type}`
      );
  ...
  const fetchChannelVideos = async (channelId: string) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/brain/youtube/channel/${channelId}/videos?limit=10`
      );
      const data = await response.json();
      if (data.success) {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>Theo dõi các kênh YouTube yêu thích và import video mới</CardDescription>
      </CardHeader>
