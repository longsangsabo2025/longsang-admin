...
async function compressImage(file: File): Promise<File> {
  ...
  console.log(
    '[Brain Upload] 🚀 Compressing large file:',
...
export class ImageMemoryAPI {
  ...
  async uploadAndAnalyze(request: UploadImageRequest): Promise<UploadImageResponse> {
    ...
    if (request.file) {
      console.log(
        '[Brain Upload] Original file:',
        request.file.name,
        (request.file.size / 1024 / 1024).toFixed(2),
        'MB',
        request.file.type
      );
  ...
  async getImage(
    imageId: string
  ): Promise<{ success: boolean; data?: ImageMemoryItem; error?: string }> {
    try {
  ...
  async updateImage(
    imageId: string,
    updates: Partial<ImageMemoryItem>
  ): Promise<{ success: boolean; data?: ImageMemoryItem; error?: string }> {
    try {
      const response = await fetch(`${this.baseUrl}/brain/images/${imageId}`, {
        method: 'PATCH',
  ...
  async findSimilar(
    imageId: string,
    limit: number = 10
  ): Promise<{ success: boolean; data: ImageMemoryItem[] }> {
    try {
      const response = await fetch(
        `${this.baseUrl}/brain/images/${imageId}/similar?limit=${limit}`
      );
      return await response.json();
    } catch (error) {
  ...
  async analyzePrompt(
    prompt: string
  ): Promise<{ success: boolean; analysis?: PromptAnalysis; error?: string }> {
    const userId = getUserId();
  ...
  ): Promise<{ success: boolean; data?: CharacterProfile; error?: string }> {
    const userId = getUserId();
  ...
  async updateCharacter(
    ...
    try {
      ...
      if (updates.referenceImageIds !== undefined)
        dbUpdates.reference_image_ids = updates.referenceImageIds;
  ...
  async createLocation(
    profile: Omit<LocationProfile, 'id' | 'userId' | 'createdAt' | 'updatedAt'>
  ): Promise<{ success: boolean; data?: LocationProfile; error?: string }> {
    const userId = getUserId();
  ...
  async createCollection(
    name: string,
    description?: string
  ): Promise<{ success: boolean; data?: ImageCollection; error?: string }> {
    const userId = getUserId();
