...
import { CheckCircle, Clock, Eye, FileDown, FileSpreadsheet, Star } from 'lucide-react';
...
import type { ActivityAction, ProductItem } from '@/hooks/library';
...
export function ExportDialog({ open, onOpenChange, productItems, logActivity }: ExportDialogProps) {
  const exportProducts = (fmt: 'csv' | 'json') => {
    ...
    logActivity(
      'export',
      `Xuất ${productItems.length} sản phẩm ra ${fmt.toUpperCase()}`,
      productItems.length
    );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Xuất danh sách sản phẩm ra file để sử dụng</DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-muted rounded-lg p-3 text-center">
              <div className="text-xl font-bold">
                {productItems.filter((p) => p.aiTags?.length).length}
              </div>
          ...
          <div className="flex gap-3">
            <Button variant="outline" className="flex-1" onClick={() => exportProducts('csv')}>
              <FileSpreadsheet className="h-4 w-4 mr-2" />
              Export CSV
            </Button>
            <Button variant="outline" className="flex-1" onClick={() => exportProducts('json')}>
