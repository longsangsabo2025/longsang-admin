...
import { Eye, Hash, Pencil } from 'lucide-react';
...
export function BulkRenameDialog({
  ...
  const applyBulkRename = () => {
    ...
    const renamedItems = productItems.map((item, idx) => {
      ...
      switch (renamePattern) {
        ...
        case 'replace': {
          ...
          newName = oldText
            ? item.name.replace(new RegExp(oldText, 'gi'), newText || '')
            : item.name;
          break;
        }
      }
    ...
    logActivity(
      'rename',
      `Đổi tên hàng loạt ${renamedItems.length} files (${renamePattern})`,
      renamedItems.length
    );
        ...
        <div className="space-y-4 py-4">
          ...
          {renamePattern !== 'date' && (
            <div className="space-y-2">
              ...
              <Input
                placeholder={
                  renamePattern === 'replace' ? 'old_text → new_text' : 'Nhập giá trị...'
                }
                value={renameValue}
