...
export function ProductsPanel({
  ...
  const batchUpdateStatus = (status: ProductItem['status']) => {
    ...
    setProductItems((prev) =>
      prev.map((item) => (selectedProductIds.includes(item.id) ? { ...item, status } : item))
    );
    const statusLabel = STATUS_CONFIG[status].label;
    logActivity(
      'batch_action',
      `Đổi ${selectedProductIds.length} files sang ${statusLabel}`,
  ...
  const approveAllDrafts = () => {
    ...
    setProductItems((prev) =>
      prev.map((item) => (item.status === 'draft' ? { ...item, status: 'approved' } : item))
    );
  ...
  const publishAllApproved = () => {
    ...
    setProductItems((prev) =>
      prev.map((item) => (item.status === 'approved' ? { ...item, status: 'published' } : item))
    );
  ...
  const handleAutoTag = async () => {
    ...
    const taggedItems = productItems.map((item) => {
      ...
      if (filename.includes('bg') || filename.includes('background'))
        detectedTags.push('background');
      if (filename.includes('icon')) detectedTags.push('icon');
      if (filename.includes('screenshot') || filename.includes('ss'))
        detectedTags.push('screenshot');
      if (filename.includes('social') || filename.includes('fb') || filename.includes('ig'))
        detectedTags.push('social-media');
        ...
        <CardContent className="p-4 h-full">
              ...
              <div className="flex gap-2 p-2 bg-gradient-to-r from-orange-500/5 to-red-500/5 rounded-lg border border-orange-500/20">
                ...
                {productItems.some((p) => p.status === 'approved') && (
                  ...
                  >
                    ...
                    {productItems.filter((p) => p.status === 'approved').length})
                  </Button>
                )}
              </div>
            )}
            ...
            <div className="flex gap-2 flex-wrap items-center">
              ...
              {batchSelectMode && selectedProductIds.length > 0 && (
                <>
                  ...
                  <Button size="sm" variant="outline" onClick={selectAllProducts}>
                    {selectedProductIds.length === productItems.length
                      ? 'Bỏ chọn tất cả'
                      : 'Chọn tất cả'}
                  </Button>
                  <DropdownMenu>
              ...
              {!batchSelectMode && productItems.length > 0 && (
                <>
                  ...
                  <Button size="sm" variant="outline" onClick={() => setShowBulkRenameDialog(true)}>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      const urls = productItems
                        .map((f) => f.url || `https://drive.google.com/file/d/${f.id}/view`)
                        .join('\n');
          ...
          {productItems.length > 0 && (
            <div className="flex gap-2 mb-4">
              ...
              <Badge
                variant="outline"
                className="cursor-pointer hover:bg-yellow-500/20 text-yellow-600"
              >
              ...
              <Badge
                variant="outline"
                className="cursor-pointer hover:bg-blue-500/20 text-blue-600"
              >
              ...
              <Badge
                variant="outline"
                className="cursor-pointer hover:bg-green-500/20 text-green-600"
              >
              ...
              <Badge
                variant="outline"
                className="cursor-pointer hover:bg-purple-500/20 text-purple-600"
              >
                    ...
                    <ContextMenu key={item.id}>
                      <ContextMenuTrigger>
                        ...
                        >
                          ...
                          <div
                            className={`absolute top-2 ${batchSelectMode ? 'left-10' : 'left-8'} z-10 opacity-0 group-hover:opacity-70 transition-opacity`}
                          >
                            {!batchSelectMode && (
                          ...
                          <div className="aspect-square bg-muted relative">
                              ...
                              <img
                                ...
                                onError={(e) => {
                                  ...
                                  if (parent && !parent.querySelector('.fallback-icon')) {
                                    ...
                                    fallback.className =
                                      'fallback-icon w-full h-full flex items-center justify-center bg-gradient-to-br ' +
                                      (item.type === 'video'
                            ...
                            {/* Status icon */}
                            <div
                              className={`absolute ${batchSelectMode ? 'top-2 right-2' : 'top-2 left-2'} p-1 rounded ${statusColors[item.status]}`}
                            >
                        ...
                        <ContextMenuItem
                          onClick={() => {
                            navigator.clipboard.writeText(
                              item.url || `https://drive.google.com/file/d/${item.id}/view`
                            );
