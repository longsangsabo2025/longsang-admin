...
const MultiAgentOrchestrator = lazy(() =>
  import('@/components/agent-center/MultiAgentOrchestrator').then((m) => ({
    default: m.MultiAgentOrchestrator,
  }))
);
const ToolsDashboard = lazy(() => import('@/components/agent-center/ToolsDashboard'));
const WorkflowOptimizer = lazy(() =>
  import('@/components/agent-center/WorkflowOptimizer').then((m) => ({
    default: m.WorkflowOptimizer,
  }))
);
const WorkflowsDashboard = lazy(() => import('@/components/agent-center/WorkflowsDashboard'));
const WorkflowTemplateLibrary = lazy(() =>
  import('@/components/workflow/WorkflowTemplateLibrary').then((m) => ({
    default: m.WorkflowTemplateLibrary,
  }))
);

// Lazy import for Sora Video (heavy component)
const SoraVideoContent = lazy(() => import('../video/SoraVideoGenerator'));

// Lazy import for Solo Founder Hub (absorbed child page)
...
const MAIN_TABS: TabConfig[] = [
  ...
  },
  {
    ...
    description: 'Solo Founder Command Center',
  },
];
