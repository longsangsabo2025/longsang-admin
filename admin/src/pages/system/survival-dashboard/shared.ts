      ...
      if (parts.length >= 3) {
        ...
        const category =
          Object.entries(categoryMap).find(([key]) =>
            categoryPart?.toLowerCase().includes(key)
          )?.[1] || 'other';
        ...
        const size =
          Object.entries(sizeMap).find(([key]) => sizePart?.toLowerCase().includes(key))?.[1] ||
          'medium';
        ...
        if (icePart) {
          if (icePart.includes('/')) {
            ...
            const score = parseInt(icePart, 10);
        ...
        if (revenuePart) {
          ...
          if (numMatch) potential_revenue = parseInt(numMatch, 10);
        }
