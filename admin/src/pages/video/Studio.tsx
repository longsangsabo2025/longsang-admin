...
export default function Studio() {
  ...
  const renderModuleButton = (module: StudioModule) => {
    ...
    return (
      ...
      >
        <div className={cn('p-2 rounded-lg', isActive ? 'bg-white/20' : 'bg-muted')}>
          {module.icon}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-medium text-sm">{module.name}</span>
            {module.status === 'coming-soon' && <Lock className="h-3 w-3" />}
            {module.status === 'beta' && (
    ...
    <div className="flex h-[calc(100vh-4rem)]">
      ...
      <div className="w-64 border-r bg-muted/30 flex flex-col">
        ...
        <div className="p-4 border-b">
          ...
          <p className="text-xs text-muted-foreground mt-1">Trung tâm sáng tạo nội dung AI</p>
        </div>
        ...
        <ScrollArea className="flex-1 p-2">
          <div className="space-y-4">
            {/* Main Hub Section */}
            <div className="space-y-1">{mainModules.map(renderModuleButton)}</div>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1">{toolModules.map(renderModuleButton)}</div>
            </div>
          </div>
        </ScrollArea>
