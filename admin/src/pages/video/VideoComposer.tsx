...
const PendingClipsPanel = ({
  onAddToComposition,
}: {
  onAddToComposition: (clip: PendingClip) => void;
}) => {
  const {
    data: clips = [],
    isLoading,
...
const ReviewDialog = ({ clip, compId, open, onOpenChange }: ReviewDialogProps) => {
  ...
  const reviewMutation = useMutation({
    mutationFn: (status: ClipReviewStatus) =>
      videoFactoryService.reviewClip(compId, clip!.id, {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Watch the video and approve or reject for composition
          </DialogDescription>
        </DialogHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className="gap-2"
            disabled={
              approvedCount === 0 || composeMutation.isPending || composition.status === 'composing'
            }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Review clips, approve, and compose into final shorts
          </p>
        </div>
