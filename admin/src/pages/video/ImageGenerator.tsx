...
import { libraryActions, useLibraryProducts, useLibraryWorkspace } from '@/hooks/library';
import type { MediaItem } from '@/hooks/library/types';
import { cn } from '@/lib/utils';
import { type AIJob, listJobs, updateJob } from '@/services/ai-jobs';
...
export default function ImageGenerator() {
  ...
  const [librarySource, setLibrarySource] = useState<
    'workspace' | 'products' | 'history' | 'favorites'
  >('workspace');
  ...
  const libraryItems = useMemo((): MediaItem[] => {
    ...
    const items = librarySource === 'workspace' ? workspaceLib.items : productLib.items;
  ...
  const addToHistory = useCallback(
    async (item: { type: string; outputUrl: string; inputUrl?: string; prompt?: string }) => {
      ...
      <div className="flex items-center justify-between p-4 border-b shrink-0">
        ...
        <div className="flex gap-2 items-center">
          {selectedImages.length > 0 && (
            <Badge variant="secondary">{selectedImages.length} ảnh đã chọn</Badge>
          )}
            ...
            <div className="border-b px-4 shrink-0 overflow-x-auto">
              <TabsList className="h-12 w-max">
                ...
                <TabsTrigger value="nano-banana" className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="ml-1 h-5 px-1.5 text-yellow-600 border-yellow-500 bg-yellow-50"
                  >
                    NEW
                  </Badge>
                </TabsTrigger>
                <TabsTrigger value="imagen-4" className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="ml-1 h-5 px-1.5 text-blue-600 border-blue-500 bg-blue-50"
                  >
                    NEW
                  </Badge>
                </TabsTrigger>
                ...
                <TabsTrigger value="brain-library" className="flex items-center gap-2">
                  ...
                  <Badge
                    variant="outline"
                    className="ml-1 h-5 px-1.5 text-purple-600 border-purple-600 bg-purple-50"
                  >
                    🧠 NEW
                  </Badge>
                </TabsTrigger>
              ...
              <TabsContent value="imagen-4" className="mt-0">
                <Imagen4
                  onGenerated={(url, prompt) =>
                    addToHistory({ type: 'imagen-4', outputUrl: url, prompt })
                  }
                />
              </TabsContent>
                                        ...
                                        onClick={async () => {
                                          ...
                                          try {
                                            const res = await fetch('/api/cloudinary/upload-url', {
                                              ...
                                              body: JSON.stringify({
                                                url: job.output_url,
                                                folder: job.job_type.includes('video')
                                                  ? 'ai-videos'
                                                  : 'ai-images',
                                                resourceType: job.job_type.includes('video')
                                                  ? 'video'
                                                  : 'image',
                                    ...
                                    <button
                                      ...
                                      onClick={async () => {
                                        const mediaItem: MediaItem = {
                                          ...
                                          mimeType: job.job_type.includes('video')
                                            ? 'video/mp4'
                                            : 'image/png',
                                        };
                                        const success = await libraryActions.addToWorkspace(
                                          mediaItem,
                                          true
                                        );
                                        if (success) {
                  ...
                  <div className="lg:col-span-1">
                    <SmartPromptAnalyzer
                      onContextReady={(context) => {
                        ...
                        toast.success(
                          `Brain tìm thấy ${context.referenceImages.length} ảnh tham chiếu!`
                        );
                      }}
