...
import { type ContentIdea as ApiContentIdea, contentIdeasApi } from '@/services/channelBuilderApi';
...
Trả về JSON array với format: [{ "title": "...", "description": "...", "platform": ["youtube", "tiktok"], "type": "video|reel|short", "tags": ["tag1", "tag2"] }]`,
  ...
  const handleScheduleIdea = async (ideaId: string) => {
    try {
      ...
      setIdeas((prev) =>
        prev.map((i) => (i.id === ideaId ? { ...i, status: 'scheduled' as const, scheduledAt } : i))
      );
