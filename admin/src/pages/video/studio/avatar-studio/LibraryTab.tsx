  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>All AI-generated avatar content</CardDescription>
      </CardHeader>
      <CardContent>
            ...
            {generatedContent.map((content) => {
              const renderMedia = () => {
                if (content.type === 'video') {
                  return (
                    <video src={content.outputUrl} className="w-full h-full object-cover" controls>
                      <track kind="captions" />
                    </video>
                  );
                }
              ...
              return (
                <Card key={content.id} className="overflow-hidden">
                  ...
                  <CardContent className="p-3">
                    <p className="text-xs text-muted-foreground truncate">{content.prompt}</p>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-xs">
                        {PLATFORM_CONFIGS[content.platform as keyof typeof PLATFORM_CONFIGS]?.icon}{' '}
                        {content.platform}
                      </span>
