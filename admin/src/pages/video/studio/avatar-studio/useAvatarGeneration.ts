...
export function useAvatarGeneration({ profile, ownerPortraits }: UseAvatarGenerationOptions) {
  ...
  const generateAvatarContent = useCallback(
    async (type: 'image' | 'video') => {
      ...
      `.trim();
        ...
        if (type === 'image') {
          ...
          if (response.ok) {
            ...
            setGeneratedContent((prev) =>
              prev.map((c) =>
                c.id === contentId ? { ...c, status: 'completed', outputUrl: data.imageUrl } : c
              )
            );
          ...
          if (videoResponse.ok) {
            ...
            setGeneratedContent((prev) =>
              prev.map((c) =>
                c.id === contentId ? { ...c, status: 'completed', outputUrl: videoUrl } : c
              )
            );
        ...
        setGeneratedContent((prev) =>
          prev.map((c) => (c.id === contentId ? { ...c, status: 'failed' } : c))
        );
