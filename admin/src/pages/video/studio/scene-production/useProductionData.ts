...
import type { EpisodeData, ProductionItem, ProductionStep, Scene } from './types';
...
export function useProductionData(settings: ProductionSettings): UseProductionDataReturn {
  ...
  const applyLocalStorageFallback = useCallback(
    (savedScenes: Scene[] | undefined, step: ProductionStep | undefined) => {
  ...
  const fetchProductionFromSupabase = useCallback(
    async (savedId: string, fallbackStep: ProductionStep): Promise<boolean> => {
      try {
