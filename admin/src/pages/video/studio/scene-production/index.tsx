...
import { Loader2, Redo2, Save, Settings2, Undo2 } from 'lucide-react';
...
import { type ExportData, exportAsCSV, exportAsJSON, exportAsPDF } from '@/services/exportService';
import {
...
import { DEFAULT_SCENE_PROMPT, DEFAULT_SETTINGS } from './types';
...
export function SceneProductionContent() {
  ...
  const toggleReference = useCallback(
    (sceneId: string, imageId: string) => {
      setScenes((prev) =>
        prev.map((s) => (s.id === sceneId ? updateSceneReferences(s, imageId) : s))
      );
    },
    [setScenes]
  );
  ...
  const handleEnhanceImagePrompt = useCallback(
    async (scene: Scene) => {
      console.log(
        '[Enhance Image] Starting with system prompt:',
        globalImageSystemPrompt?.slice(0, 100) || '(empty)'
      );
  ...
  const handleEnhanceVideoPrompt = useCallback(
    async (scene: Scene) => {
  ...
  const handleConfirmBrainPicker = useCallback(
    (ids: string[]) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Main Content */}
      <div className="flex-1 overflow-hidden p-4">{renderStep()}</div>
