...
import type { ReferenceImage, Scene } from './types';
...
export function useSceneGeneration({
  ...
  const enhancePromptWithAI = useCallback(
    async (prompt: string, context: 'image' | 'video', customPrompt?: string): Promise<string> => {
      try {
        const systemPrompt =
          customPrompt || (context === 'image' ? globalImageSystemPrompt : globalVideoSystemPrompt);

        console.log('[AI Enhance] Context:', context);
        console.log('[AI Enhance] System prompt length:', systemPrompt?.length || 0);
        console.log(
          '[AI Enhance] System prompt preview:',
          systemPrompt?.slice(0, 100) || '(empty)'
        );
  ...
  const generateSceneImage = useCallback(
    async (scene: Scene, attempt = 1, maxRetries = 3) => {
      console.log(
        `[Image Gen] Attempt ${attempt}/${maxRetries} for scene:`,
        scene.id,
        scene.number
      );
      ...
      try {
        ...
        const refImages = brainImages.filter((img) => scene.referenceImageIds.includes(img.id));
        console.log(
        ...
        if (data.success && (data.output || data.image_url)) {
          ...
          setScenes((currentScenes) => {
            const updatedScenes = currentScenes.map((s) =>
              s.id === scene.id
                ? { ...s, generatedImageUrl: imageUrl, status: 'image_ready' as const }
                : s
            );
            localStorage.setItem(
              SCENE_PRODUCTION_KEY,
              JSON.stringify({
                productionId,
        ...
        if (attempt < maxRetries) {
          ...
          toast.info(`⚠️ Retry ${attempt}/${maxRetries} sau ${backoffMs / 1000}s...`, {
            duration: backoffMs,
          });
      ...
      try {
        ...
        if (data.video_url) {
          ...
          setScenes((currentScenes) => {
            const updatedScenes = currentScenes.map((s) =>
              s.id === scene.id
                ? { ...s, generatedVideoUrl: videoUrl, status: 'video_ready' as const }
                : s
            );
            localStorage.setItem(
              SCENE_PRODUCTION_KEY,
          ...
          toast.info(`Scene ${scene.number}: Đang tạo video VEO 3... (max 3 phút)`, {
            duration: 5000,
          });
          ...
          const pollResult = async (): Promise<string | null> => {
            while (attempts < maxAttempts) {
              ...
              if (elapsedSeconds >= 180) {
                ...
                throw new Error(
                  'Video generation timeout (3 minutes). Try simpler prompt or shorter video.'
                );
              }
              ...
              try {
                ...
                const statusRes = await fetch(
                  `/api/veo-gemini/status/${encodeURIComponent(data.prediction_id)}`
                );
                ...
                if (statusData.status === 'succeeded' && statusData.video_url) {
                  ...
                  throw new Error(
                    'Google VEO không tạo được video. Có thể prompt vi phạm policy hoặc quá phức tạp.'
                  );
                } else if (statusData.status === 'processing') {
                  // Still processing, continue polling
                  console.log(
                    `[VEO Polling] Still processing... (attempt ${attempts}/${maxAttempts})`
                  );
                }
              } catch (pollError: any) {
        ...
        if (errorMessage.includes('policy') || errorMessage.includes('rejected')) {
          toast.error(`Scene ${scene.number}: Google từ chối tạo video (có thể vi phạm policy)`, {
            duration: 8000,
          });
        } else if (errorMessage.includes('Timeout')) {
          toast.error(`Scene ${scene.number}: Timeout - VEO mất quá lâu (>3 phút)`, {
            duration: 6000,
          });
        } else {
