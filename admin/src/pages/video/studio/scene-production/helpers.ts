 ...
 */
export const buildVideoGenRequest = (scene: Scene, aspectRatio: string): VideoGenRequest => {
  return {
...
export const isSceneValid = (scene: Scene): boolean => {
  return !!(scene.description?.trim() && scene.visualPrompt?.trim() && scene.duration > 0);
};
...
export const updateSceneInArray = (
  ...
  return scenes.map((s) => (s.id === sceneId ? { ...s, ...updates } : s));
};
