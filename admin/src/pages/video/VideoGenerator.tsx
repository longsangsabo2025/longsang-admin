...
import { ArrowLeft, Film, Megaphone, Monitor, Sparkles, Video } from 'lucide-react';
...
export default function VideoGenerator() {
  ...
  const updateActivityLog = useCallback((id: string, updates: Partial<ActivityLogItem>) => {
    setActivityLog((prev) => prev.map((item) => (item.id === id ? { ...item, ...updates } : item)));
  }, []);

  // Handle video generated callback
  const handleVideoGenerated = useCallback(
    (video: { url: string; prompt: string; type: string }) => {
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-4 lg:w-auto lg:inline-flex">
            <TabsTrigger value="veo-video" className="flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className="ml-1 h-5 px-1.5 text-purple-600 border-purple-500 bg-purple-50"
              >
                NEW
              </Badge>
            </TabsTrigger>
            <TabsTrigger value="local-wan" className="flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className="ml-1 h-5 px-1.5 text-green-600 border-green-500 bg-green-50"
              >
                FREE
              </Badge>
            </TabsTrigger>
            ...
            <TabsTrigger value="ugc-ads" className="flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className="ml-1 h-5 px-1.5 text-pink-600 border-pink-600 bg-pink-50"
              >
                HOT
              </Badge>
            </TabsTrigger>
          ...
          <TabsContent value="image-to-video" className="mt-0">
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>Biến ảnh tĩnh thành video động với AI</CardDescription>
              </CardHeader>
              <CardContent>
                <ImageToVideo
          ...
          <TabsContent value="ugc-ads" className="mt-0">
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>Tạo video quảng cáo UGC style với AI avatar</CardDescription>
              </CardHeader>
              <CardContent>
