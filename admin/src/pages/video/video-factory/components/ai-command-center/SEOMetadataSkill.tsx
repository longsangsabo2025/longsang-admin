      ...
      <Card className="border-cyan-500/20">
        <CardHeader className="pb-3">
          <CardTitle className="text-base flex items-center gap-2">
            🏷️ SEO & Caption Generator
          </CardTitle>
          <CardDescription>
            Generate optimized titles, descriptions, hashtags, and captions for any content
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Textarea
                placeholder="Describe your video/image content..."
                value={seoDesc}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  placeholder="The AI prompt used to generate..."
                  value={seoPrompt}
          ...
          <div className="flex flex-wrap gap-2">
            {['tiktok', 'youtube', 'instagram', 'twitter'].map((p) => (
              <Button
                key={p}
                size="sm"
                variant={seoPlatforms.includes(p) ? 'default' : 'outline'}
                className="text-xs gap-1"
                onClick={() =>
                  setSeoPlatforms((prev: string[]) =>
                    prev.includes(p) ? prev.filter((x) => x !== p) : [...prev, p]
                  )
                }
              >
                {p === 'tiktok' ? '🎵' : p === 'youtube' ? '📺' : p === 'instagram' ? '📸' : '🐦'}{' '}
                {p}
              </Button>
            ))}
          </div>
          <Button
            ...
            disabled={loading || !seoDesc.trim() || !seoNiche.trim()}
            onClick={onGenerate}
          >
            {loading ? (
              <>
              ...
              <>
                <Hash className="h-4 w-4" /> Generate SEO & Captions
              </>
            )}
          </Button>
        </CardContent>
        ...
        <div className="space-y-3">
          ...
          <Card className="border-dashed">
            <CardContent className="pt-4 pb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <Badge
                  className={`${(seoResult.seo_score || 0) >= 80 ? 'bg-green-600' : 'bg-orange-500'}`}
                >
            ...
            <CardContent className="space-y-3">
              {seoResult.hashtags?.primary && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {seoResult.hashtags.primary.map((h, i) => (
              ...
              {seoResult.hashtags?.trending && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {seoResult.hashtags.trending.map((h, i) => (
              ...
              {seoResult.hashtags?.per_platform &&
                Object.entries(seoResult.hashtags.per_platform).map(([platform, tags]) => (
                  <div key={platform}>
          ...
          <div className="grid md:grid-cols-2 gap-3">
              ...
              <Card>
                ...
                <CardContent className="space-y-1">
                  {seoResult.hooks.map((h, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-1.5 text-xs rounded bg-muted/30"
                    >
                      <span>{h}</span>
                      <Button
                        variant="ghost"
            ...
            {seoResult.cta_options?.length > 0 && (
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">📣 CTA Options</CardTitle>
                </CardHeader>
                <CardContent className="space-y-1">
