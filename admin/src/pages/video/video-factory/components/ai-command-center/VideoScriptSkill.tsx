    ...
    <div className="space-y-4">
      <Card className="border-purple-500/20">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Complete scripts with voiceover text, scene-by-scene AI prompts, and SEO metadata
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <Input
                placeholder="e.g. How AI is changing filmmaking..."
                value={scTopic}
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                value={scNiche}
                onChange={(e) => setScNiche(e.target.value)}
              />
            </div>
          </div>
          ...
          >
            {loading ? (
              ...
              <>
                <FileText className="h-4 w-4" /> Generate Video Script
              </>
            )}
          </Button>
        </CardContent>
        ...
        <div className="space-y-4">
          <Card className="border-purple-500/10">
            <CardHeader className="pb-2">
              ...
              <CardDescription>
                {scResult.total_duration} • {scResult.style} • {scResult.scenes?.length || 0} scenes
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* Full Voiceover */}
                ...
                <div className="rounded-md bg-purple-500/5 border border-purple-500/20 p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Button
                      ...
                      onClick={() => copyToClipboard(scResult.voiceover_full_text)}
                    >
              ...
              {scResult.scenes?.map((scene) => (
                <div
                  ...
                  className="rounded-md bg-muted/40 p-3 space-y-1 border"
                >
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-xs text-muted-foreground line-clamp-1 flex-1">
                      <strong>AI Prompt:</strong> {scene.ai_prompt}
                    </p>
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-6 w-6 p-0 shrink-0"
                      onClick={() => copyToClipboard(scene.ai_prompt)}
                    >
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 border-t">
                {scResult.music_suggestion && (
                  <div className="text-xs">
                    <strong>🎵 Music:</strong> {scResult.music_suggestion}
                  </div>
                )}
                {scResult.thumbnail_prompt && (
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <Button
                      ...
                      size="sm"
                      className="h-6 w-6 p-0 shrink-0"
                      onClick={() => copyToClipboard(scResult.thumbnail_prompt)}
                    >
                      <Copy className="h-3 w-3" />
                    </Button>
                  </div>
