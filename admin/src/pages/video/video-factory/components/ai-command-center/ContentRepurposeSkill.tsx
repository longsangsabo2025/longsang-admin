    ...
    <div className="space-y-4">
      <Card className="border-green-500/20">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Take 1 content idea → auto-adapt for 6 platforms with optimized prompts
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                value={rpNiche}
                onChange={(e) => setRpNiche(e.target.value)}
              />
            </div>
          </div>
          <Button
            className="w-full gap-2"
            disabled={loading || !rpPrompt.trim()}
            onClick={onGenerate}
          >
            {loading ? (
              <>
              ...
              <>
                <RefreshCw className="h-4 w-4" /> Repurpose to All Platforms
              </>
            )}
          </Button>
        </CardContent>
        ...
        <div className="space-y-3">
          <Card className="border-dashed">
            <CardContent className="pt-4 pb-3 flex items-center justify-between">
              <p className="text-sm">
                <strong>Reach Multiplier:</strong> {rpResult.total_reach_multiplier}
              </p>
          ...
          {rpResult.variations?.map((v, idx) => (
            <Card key={idx} className="border-green-500/10">
              <CardContent className="pt-4 pb-3 space-y-2">
                    ...
                    <Badge className="text-xs">
                      {v.platform === 'tiktok'
                        ...
                        : v.platform === 'youtube_shorts'
                          ? '📺'
                          : v.platform === 'youtube_long'
                            ? '🎬'
                            : v.platform === 'instagram_reels'
                              ? '📸'
                              : v.platform === 'instagram_post'
                                ? '🖼️'
                                : '🐦'}{' '}
                      {v.platform}
                    </Badge>
                ...
                {v.adaptation_notes && (
                  <p className="text-[10px] text-muted-foreground">📝 {v.adaptation_notes}</p>
                )}
                {v.hashtags?.length > 0 && (
