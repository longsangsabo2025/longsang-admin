    ...
    <div className="space-y-4">
      <Card className="border-orange-500/20">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Discover trending topics, viral patterns, and content opportunities in your niche
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                value={tsNiche}
                onChange={(e) => setTsNiche(e.target.value)}
              />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                max={10}
                value={tsCount}
                onChange={(e) => setTsCount(parseInt(e.target.value, 10) || 5)}
              />
            </div>
          </div>
          <Button
            className="w-full gap-2"
            disabled={loading || !tsNiche.trim()}
            onClick={onGenerate}
          >
            {loading ? (
              <>
                <Loader2 className="h-4 w-4 animate-spin" /> Scouting Trends...
              </>
            ) : (
              <>
                <Target className="h-4 w-4" /> Scout Trends
              </>
            )}
          </Button>
        </CardContent>
            ...
            <Card className="border-dashed">
              <CardContent className="pt-4 pb-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <Badge
                    className={`${tsResult.trend_score >= 70 ? 'bg-green-600' : 'bg-orange-500'}`}
                  >
                ...
                {tsResult.recommended_priority?.length > 0 && (
                  <p className="text-xs text-muted-foreground">
                    Priority: {tsResult.recommended_priority.join(' → ')}
                  </p>
                )}
              </CardContent>
            </Card>
          ...
          {tsResult.trends?.map((trend, idx) => (
            <Card key={idx} className="border-orange-500/10">
                ...
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base flex items-center gap-2">
                    <span className="text-lg">
                      {trend.category === 'evergreen'
                        ? '🌲'
                        : trend.category === 'seasonal'
                          ? '🎄'
                          : trend.category === 'emerging'
                            ? '🚀'
                            : trend.category === 'niche_specific'
                              ? '🔬'
                              : '🔗'}
                    </span>
                    {trend.trend_name}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      className={`text-xs ${trend.virality_score >= 8 ? 'bg-red-600' : 'bg-orange-500'}`}
                    >
              ...
              <CardContent className="space-y-3">
                <p className="text-xs">
                  <strong>Why trending:</strong> {trend.why_trending}
                </p>
                <p className="text-xs">
                  <strong>Timing:</strong> {trend.timing}
                </p>
                ...
                {trend.content_ideas?.length > 0 && (
                  <div className="space-y-2 pt-2 border-t">
                    ...
                    {trend.content_ideas.map((idea, iIdx) => (
                      <div
                        key={iIdx}
                        className="flex items-start gap-2 p-2 rounded bg-muted/30 border text-xs"
                      >
                        <div className="flex-1">
                          <span className="font-medium">{idea.title}</span>
                          {idea.hook && (
                            <span className="text-muted-foreground ml-2">— {idea.hook}</span>
                          )}
                        ...
                        <Button
                          ...
                          onClick={() => copyToClipboard(idea.prompt)}
                        >
