...
import { Image as ImageIcon, Rocket, Shield, Trophy, Video } from 'lucide-react';
...
import { IMAGE_MODELS_SET, MODEL_INTELLIGENCE, TIER_CONFIG } from '../../shared-data';
        ...
        <Table>
          ...
          <TableBody>
            {modelPerf.map((perf, idx) => {
              ...
              const tier =
                meta?.tier || (IMAGE_MODELS_SET.has(perf.model) ? 'image' : 'experimental');
              const tierCfg = TIER_CONFIG[tier];

              return (
                <TableRow
                  key={perf.model}
                  className={idx === 0 && perf.totalJobs > 0 ? 'bg-yellow-500/5' : ''}
                >
                  <TableCell className="font-bold text-muted-foreground">
                    {idx === 0 && perf.totalJobs > 0 ? (
                  ...
                  <TableCell>
                    ...
                    {meta && (
                      <p className="text-xs text-muted-foreground mt-0.5">{meta.provider}</p>
                    )}
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-right font-mono">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {perf.failed > 0 && (
                        <span className="text-xs text-destructive">+{perf.failed}⛔</span>
                      )}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {meta ? (
                      <Badge
                        variant="outline"
                        className={
                          meta.costType === 'unlimited'
                            ? 'text-green-600 border-green-500/30 text-xs'
                            : meta.costType === 'free'
                              ? 'text-blue-600 border-blue-500/30 text-xs'
                              : 'text-orange-600 border-orange-500/30 text-xs'
                        }
                      >
                        {meta.costType === 'unlimited'
                          ? '♾️ Unlimited'
      ...
      <Card className="border-yellow-500/30 bg-yellow-500/5">
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            <Rocket className="h-4 w-4 text-yellow-600" />🧠 Elon Strategy — Focus on the BEST
          </CardTitle>
          <CardDescription>
            "If you're using 30 models, you're not using any of them well." — 80/15/5 Rule
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div key={perf.model} className="p-3 rounded-lg border bg-card space-y-2">
                    ...
                    <div className="text-xs space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">{perf.completed}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Avg Time</span>
                        <span className="font-mono">{perf.avgGenTime}s</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Success</span>
                        <span className="font-mono">{perf.successRate}%</span>
                      </div>
                    </div>
                  </div>
                );
