...
import { Activity, CheckCircle2, Clock, Eye, Loader2, Rocket, Trophy, Zap } from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Same prompt → multiple models → compare results in real-time
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {allModelIds
                .filter((id) => MODEL_INTELLIGENCE[id]?.type === 'video')
                .map((id) => {
            ...
            <div className="flex flex-wrap gap-2">
              {allModelIds
                .filter((id) => MODEL_INTELLIGENCE[id]?.type === 'image')
                .map((id) => {
                  const meta = MODEL_INTELLIGENCE[id];
                  const isSelected = abTestModels.includes(id);
                  return (
          ...
          <Button
            ...
            disabled={
              abTestPrompt.trim().length < 5 || abTestModels.length < 2 || abTestMutation.isPending
            }
            onClick={() =>
              abTestMutation.mutate({ prompt: abTestPrompt, models: abTestModels as VideoModel[] })
            }
          >
      ...
      {!abTestId && (
        <Card className="border-dashed">
          <CardContent className="pt-6 pb-6 flex flex-col items-center text-center text-muted-foreground">
            ...
            <p className="text-sm">
              Configure a prompt and select models above to start a comparison test
            </p>
          </CardContent>
        </Card>
      )}
