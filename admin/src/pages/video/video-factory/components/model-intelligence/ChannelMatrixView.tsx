...
import { BarChart3, TrendingUp, Tv } from 'lucide-react';
                        ...
                        <TableCell>
                          {perf && perf.totalJobs > 0 ? (
                            <div className="flex items-center gap-2 text-xs">
                              ...
                              <Badge
                                variant={perf.successRate >= 90 ? 'default' : 'secondary'}
                                className="text-xs"
                              >
                                {perf.successRate}%
                              </Badge>
              ...
              <div className="space-y-3">
                {modelPerf
                  ...
                  .map((perf) => {
                    ...
                    const pct =
                      totalProduction > 0
                        ? Math.round((perf.completed / totalProduction) * 100)
                        : 0;
                    return (
      ...
      <Card className="border-primary/20">
        ...
        <CardContent>
          <div className="grid md:grid-cols-2 gap-3">
            {[
              {
                need: '⚡ Max Speed Production',
                rec: 'MiniMax Hailuo',
                reason: '~22s, unlimited, consistent',
              },
              {
                need: '🎬 Cinematic Hero Content',
                rec: 'Kling 3.0',
                reason: '4K + audio + multi-shot 15s',
              },
              {
                need: '🎨 Artistic / Editorial',
                rec: 'Soul 2.0',
                reason: '20+ presets, culture-native',
              },
              {
                need: '🖼️ Best Thumbnails',
                rec: 'Nano Banana Pro',
                reason: '<10s, 4K, perfect text',
              },
              { need: '📝 Text in Images', rec: 'Flux 2 Pro', reason: 'Best text rendering' },
              {
                need: '🧪 Complex Narratives',
                rec: 'Sora 2',
                reason: 'World knowledge, 20s duration',
              },
            ].map((item) => (
              <div
                key={item.need}
                className="flex items-start gap-2 p-2 rounded bg-muted/40 border text-xs"
              >
