...
import { Activity, Clock, Gauge, Image as ImageIcon, Video } from 'lucide-react';
...
const ModelCard = ({
  ...
  perf:
    | { totalJobs: number; completed: number; avgGenTime: number; successRate: number }
    | undefined;
      ...
      {perf && perf.totalJobs > 0 && (
        <div className="text-xs p-2 rounded bg-primary/5 border border-primary/20 space-y-1">
          <div className="font-medium flex items-center gap-1">
            <Activity className="h-3 w-3" /> Production Data
          </div>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold flex items-center gap-2">
          <Video className="h-4 w-4" /> Video Models
        </h3>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.values(MODEL_INTELLIGENCE)
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold flex items-center gap-2">
          <ImageIcon className="h-4 w-4" /> Image Models
        </h3>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {Object.values(MODEL_INTELLIGENCE)
