            ...
            <Table>
                ...
                <TableRow>
                  ...
                  {compareModels.map((id) => (
                    <TableHead key={id} className="text-center min-w-[160px]">
                      ...
                      <Badge
                        variant="outline"
                        className={`text-[10px] mt-1 ${TIER_CONFIG[MODEL_INTELLIGENCE[id]?.tier || 'experimental'].color}`}
                      >
                        {TIER_CONFIG[MODEL_INTELLIGENCE[id]?.tier || 'experimental'].icon}{' '}
                        {TIER_CONFIG[MODEL_INTELLIGENCE[id]?.tier || 'experimental'].label}
                      </Badge>
                    </TableHead>
                  ))}
                </TableRow>
              ...
              <TableBody>
                  ...
                  {compareModels.map((id) => (
                    <TableCell key={id} className="text-center">
                      <Badge variant="secondary" className="text-xs">
                        {MODEL_INTELLIGENCE[id]?.type === 'video' ? '🎬' : '🖼️'}{' '}
                        {MODEL_INTELLIGENCE[id]?.type}
                      </Badge>
                    </TableCell>
                  ))}
                </TableRow>
                ...
                <TableRow>
                  ...
                  {compareModels.map((id) => (
                    <TableCell key={id} className="text-center text-sm">
                      {MODEL_INTELLIGENCE[id]?.provider}
                    </TableCell>
                  ))}
                </TableRow>
                {/* Max Resolution */}
                <TableRow>
                  ...
                  {compareModels.map((id) => (
                    <TableCell key={id} className="text-center font-mono text-sm">
                      {MODEL_INTELLIGENCE[id]?.maxResolution}
                    </TableCell>
                  ))}
                </TableRow>
                {/* Max Duration */}
                <TableRow>
                  ...
                  {compareModels.map((id) => (
                    <TableCell key={id} className="text-center font-mono text-sm">
                      {MODEL_INTELLIGENCE[id]?.maxDuration || '—'}
                    </TableCell>
                  ))}
                </TableRow>
                {/* Speed */}
                ...
                <TableRow>
                  ...
                  {compareModels.map((id) => (
                    <TableCell key={id} className="text-center">
                      <Badge
                        variant={
                          MODEL_INTELLIGENCE[id]?.costType === 'unlimited' ? 'default' : 'secondary'
                        }
                        className="text-xs"
                      >
                        {MODEL_INTELLIGENCE[id]?.costType === 'unlimited'
                          ? '♾️ Unlimited'
                          : MODEL_INTELLIGENCE[id]?.costType === 'credits'
                            ? '💎 Credits'
                            : MODEL_INTELLIGENCE[id]?.costType}
                      </Badge>
                    </TableCell>
                  ))}
                </TableRow>
                ...
                <TableRow className="bg-muted/30">
                  ...
                  {compareModels.map((id) => (
                    <TableCell key={id} className="text-center text-xs text-muted-foreground">
                      Live Data ↓
                    </TableCell>
                  ))}
                </TableRow>
                  ...
                  {compareModels.map((id) => {
                    ...
                    return (
                      <TableCell key={id} className="text-center">
                        <Badge
                          variant={
                            rate >= 90 ? 'default' : rate >= 70 ? 'secondary' : 'destructive'
                          }
                          className="font-mono"
                        >
                          {rate}%
                      ...
                      <div className="flex flex-wrap gap-1 justify-center">
                        ...
                        {(MODEL_INTELLIGENCE[id]?.features.length || 0) > 4 && (
                          <Badge variant="outline" className="text-[10px]">
                            +{MODEL_INTELLIGENCE[id].features.length - 4}
                          </Badge>
                        )}
                      </div>
                    </TableCell>
                ...
                <TableRow>
                  ...
                  {compareModels.map((id) => (
                    <TableCell key={id} className="text-xs text-center text-muted-foreground">
                      {MODEL_INTELLIGENCE[id]?.bestFor}
                    </TableCell>
                  ))}
                </TableRow>
              </TableBody>
