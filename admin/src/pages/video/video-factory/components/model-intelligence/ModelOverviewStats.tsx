...
import { AlertCircle, Brain, Crown, Gauge, Image as ImageIcon, Trophy, Video } from 'lucide-react';
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-bold truncate">
                  {topModel ? MODEL_INTELLIGENCE[topModel.model]?.name || topModel.model : '—'}
                </p>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-sm font-medium flex items-center gap-1.5">
                    <Video className="h-3.5 w-3.5" /> Video Slots
                  </span>
                  <span className="text-sm text-muted-foreground">
                    {capacityData.video.current}/{capacityData.video.limit}
                  </span>
                </div>
                <Progress
                  value={(capacityData.video.current / capacityData.video.limit) * 100}
                  className="h-2"
                />
              </div>
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {capacityData.image.current}/{capacityData.image.limit}
                  </span>
                </div>
                <Progress
                  value={(capacityData.image.current / capacityData.image.limit) * 100}
                  className="h-2"
                />
              </div>
            </div>
              ...
              {alerts.map((alert, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  {alert.recommendation && (
                    <Badge
                      variant="outline"
                      className="text-[10px] text-green-600 border-green-500/30 gap-0.5"
                    >
