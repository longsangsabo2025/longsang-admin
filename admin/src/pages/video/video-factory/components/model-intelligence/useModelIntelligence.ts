...
export function useModelIntelligence(): UseModelIntelligenceReturn {
  ...
  const abTestMutation = useMutation({
    mutationFn: (req: { prompt: string; models: string[] }) =>
      videoFactoryService.createABTest(req),
    onSuccess: (data) => {
      setAbTestId(data.test_id);
      toast({
        title: '🧪 A/B Test Started',
        description: `Testing ${data.count} models: ${data.models.join(', ')}`,
      });
    },
    onError: (err: Error) =>
      toast({ title: 'A/B Test Failed', description: err.message, variant: 'destructive' }),
  });
  ...
  const modelPerf = useMemo<ModelPerfStats[]>(() => {
    ...
    for (const job of allJobs) {
      ...
      if (!map.has(model))
        map.set(model, { total: 0, completed: 0, failed: 0, genTimes: [], lastUsed: null });
    ...
    for (const modelId of Object.keys(MODEL_INTELLIGENCE)) {
      if (!map.has(modelId))
        map.set(modelId, { total: 0, completed: 0, failed: 0, genTimes: [], lastUsed: null });
    }
    ...
    return Array.from(map.entries())
      .map(([model, data]) => ({
        ...
        failed: data.failed,
        avgGenTime:
          data.genTimes.length > 0
            ? Math.round((data.genTimes.reduce((a, b) => a + b, 0) / data.genTimes.length) * 10) /
              10
            : 0,
  ...
  const avgTime =
    modelPerf.filter((m) => m.avgGenTime > 0).reduce((s, m) => s + m.avgGenTime, 0) /
    Math.max(modelPerf.filter((m) => m.avgGenTime > 0).length, 1);
