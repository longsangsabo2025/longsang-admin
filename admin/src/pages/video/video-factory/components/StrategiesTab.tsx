...
import { BookOpen, Loader2, Plus, Rocket, Sparkles, Wand2, Zap } from 'lucide-react';
...
import { type ContentStrategy, videoFactoryService } from '@/services/video-factory.service';
...
export const StrategiesTab = () => {
  ...
  const generateMutation = useMutation({
    mutationFn: ({ id, count }: { id: string; count: number }) =>
      videoFactoryService.generateFromStrategy(id, count),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['video-factory'] });
      toast({
        title: `🚀 Generated ${data.generated} jobs`,
        description: 'Added to production queue',
      });
  ...
  const enhanceMutation = useMutation({
    mutationFn: ({ id, count }: { id: string; count: number }) =>
      videoFactoryService.enhanceStrategy(id, count),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['video-factory'] });
      toast({
        title: `🧠 AI Enhanced — ${data.count || 0} prompts created`,
        description: data.strategy_name || '',
      });
      setEnhancingId(null);
    },
    onError: (err: Error) =>
      toast({ title: 'Enhance Error', description: err.message, variant: 'destructive' }),
  });
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <Select
            value={filterChannel || '__none__'}
            onValueChange={(v) => setFilterChannel(v === '__none__' ? '' : v)}
          >
      ...
      {strategies.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {strategies.map((strategy: ContentStrategy) => {
            const channelName =
              channels.find((c) => c.id === strategy.channel_id)?.name || 'Unknown';
            const typeConf = contentTypeConfig[strategy.content_type] || {
              icon: '📝',
              color: 'bg-gray-500/10 text-gray-600',
            };
            const freqConf = frequencyConfig[strategy.frequency] || {
                    ...
                    {genStrategyId === strategy.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            generateMutation.mutate({ id: strategy.id, count: genCount })
                          }
                        >
                        ...
                        <Button size="sm" variant="ghost" onClick={() => setGenStrategyId(null)}>
                          ✕
                        </Button>
                      </div>
                    ) : (
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Define a reusable prompt template for batch content generation
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label>Channel *</Label>
                <Select
                  value={newStrategy.channel_id}
                  onValueChange={(v) => setNewStrategy({ ...newStrategy, channel_id: v })}
                >
                  <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setNewStrategy({ ...newStrategy, prompt_template: e.target.value })
                }
                placeholder="A cinematic shot of {subject} in {setting}, dramatic lighting, 4K quality"
                rows={4}
              />
              <p className="text-xs text-muted-foreground">
                Use {'{variable}'} for dynamic parts. Variables will be filled when generating.
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  value={newStrategy.content_type}
                  onValueChange={(v) => setNewStrategy({ ...newStrategy, content_type: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setNewStrategy({ ...newStrategy, frequency: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewStrategy({ ...newStrategy, priority: parseInt(e.target.value, 10) || 5 })
                  }
                />
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button
              disabled={
                !newStrategy.name ||
                !newStrategy.channel_id ||
                !newStrategy.prompt_template ||
