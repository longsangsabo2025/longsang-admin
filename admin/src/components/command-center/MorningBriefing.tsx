...
import { useAgentStats, useRegistryAgents } from '@/hooks/use-agent-company';
import {
  useAgents,
  useMarkBriefingRead,
  usePendingDecisions,
...
export function MorningBriefing() {
    ...
    registryAgents.length > 0
      ? registryAgents
          ...
          .map((a) => ({
            ...
            icon: <Zap className="h-4 w-4" />,
            status:
              a.status === 'busy' ? ('active' as const) : (a.status as 'active' | 'idle' | 'error'),
  ...
  const handleReject = (itemId: string) => {
    ...
    toast({
      title: 'Rejected',
      description: item?.title || 'Item rejected',
      variant: 'destructive',
    });
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-500">
                    {registryStats?.active ?? agents.filter((a) => a.status === 'active').length}/
                    {registryStats?.total ?? agents.length}
                  </p>
                </div>
