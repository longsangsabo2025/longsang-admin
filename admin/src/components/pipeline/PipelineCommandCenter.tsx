...
const AGENTS = [
  { id: 'harvester', name: 'Harvester', icon: Search, desc: 'Content extraction from YouTube/web' },
  {
    ...
    name: 'Brain Curator',
    icon: Brain,
    desc: 'Knowledge curation + book refs',
  },
...
OUTPUT: JSON with title, script[] (section, text), estimatedMinutes`,
  'voice-producer':
    'You are a voice production assistant. Help prepare scripts for TTS conversion.',
...
export function PipelineCommandCenter() {
  ...
  const checkTTS = useCallback(async () => {
    ...
    try {
      const r = await fetch(`${voiceConfig.tts_url}/v1/health`, {
  ...
  useEffect(() => {
    ...
    loadHistory();
    checkTTS();
  }, [loadConfigs, loadHistory, checkTTS]);

  useEffect(() => {
    const ch = supabase
      .channel('cmd-center-realtime')
  ...
  const updateAgent = (agentId: string, field: string, value: unknown) => {
    setAgentConfigs((prev) =>
      prev.map((c) => (c.agent_id === agentId ? { ...c, [field]: value } : c))
    );
  };
  ...
  const buildTriggerBody = (
    type: 'topic' | 'url',
    value: string,
    opts?: { fromStage?: number; pipelineId?: string }
  ) => {
    const body: Record<string, unknown> = type === 'url' ? { videoUrl: value } : { topic: value };
    body.configs = Object.fromEntries(
      agentConfigs.map((c) => [
        c.agent_id,
        {
          model: c.model,
  ...
  const triggerSingle = async (
    ...
    const endpoint = opts?.pipelineId
      ? `${API_BASE}/youtube-crew/resume`
      : `${API_BASE}/youtube-crew/trigger`;
  ...
  const runQueue = async () => {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      try {
        ...
        setQueue((prev) =>
          prev.map((q, idx) => (idx === i ? { ...q, status: ok ? 'done' : 'failed' } : q))
        );
        if (ok) loadHistory();
        ...
        <TabsContent value="run" className="space-y-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              ...
              <CardDescription>
                Template mặc định được áp dụng tự động. Nhập 1 topic/URL → RUN, hoặc thêm vào hàng
                đợi.
              </CardDescription>
            </CardHeader>
                ...
                <Input
                  ...
                  placeholder={
                    triggerType === 'url'
                      ...
                      : 'Bí mật giấc ngủ mà 99% người không biết'
                  }
                  className="flex-1"
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm flex items-center gap-2">
                    <ListPlus className="h-4 w-4" />
                    Hàng đợi ({queue.filter((q) => q.status === 'queued').length} chờ /{' '}
                    {queue.length} tổng)
                  </CardTitle>
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={runQueue}
                      disabled={
                        batchRunning || queue.filter((q) => q.status === 'queued').length === 0
                      }
                      className="gap-1"
                    >
                      {batchRunning ? (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-xs">
                          {run.pipeline_id?.slice(-8) || run.id?.slice(0, 8)}
                        </span>
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        <span className="flex items-center gap-1">
                          <DollarSign className="h-3 w-3" />${(run.total_cost || 0).toFixed(4)}
                        </span>
                        <span className="flex items-center gap-1">
                          <Timer className="h-3 w-3" />
                          {run.total_duration_ms
                            ? `${(run.total_duration_ms / 1000).toFixed(0)}s`
                            : '—'}
                        </span>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <Slider
                          ...
                          onValueChange={(v) => updateAgent(a.id, 'temperature', v[0])}
                        />
                      </div>
                      <div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div>
                  ...
                  <Input
                    className="mt-1 text-sm"
                    placeholder="D:/path/to/voice_ref.wav"
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onValueChange={(v) => setVoiceConfig((c) => ({ ...c, speed: v[0] }))}
                  />
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  <Slider
                    ...
                    max={500}
                    step={25}
                    value={[voiceConfig.chunk_size]}
                    onValueChange={(v) => setVoiceConfig((c) => ({ ...c, chunk_size: v[0] }))}
                  />
                  <p className="text-[10px] text-muted-foreground mt-1">
                    Smaller = more stable, slower. 200-300 recommended.
                  </p>
                </div>
                <div>
                  <Label className="text-xs">
                    Min Whisper Similarity: {voiceConfig.min_similarity}
                  </Label>
                  <Slider
                    className="mt-2"
                    min={0.3}
                    max={0.9}
                    step={0.05}
                    value={[voiceConfig.min_similarity]}
                    onValueChange={(v) => setVoiceConfig((c) => ({ ...c, min_similarity: v[0] }))}
                  />
                  <p className="text-[10px] text-muted-foreground mt-1">
                    Lower = more lenient. 0.5 good for Vietnamese.
                  </p>
                </div>
                <div>
                  ...
                  <Input
                    type="number"
                    className="mt-1 text-sm"
                    value={voiceConfig.max_consecutive_fails}
                    onChange={(e) =>
                      setVoiceConfig((v) => ({
                        ...v,
                        max_consecutive_fails: Number(e.target.value),
                      }))
                    }
                  />
                </div>
          ...
          {history.length === 0 ? (
            ...
            history.map((run) => {
              ...
              const stages: StageInfo[] =
                (run.stages as StageInfo[] | undefined) ||
                AGENTS.map((a, idx) => ({
                  name: a.name,
                  status:
                    run.failed_stage !== undefined && idx === run.failed_stage
                      ? ('failed' as const)
                      : run.last_completed_stage !== undefined && idx <= run.last_completed_stage
                        ? ('completed' as const)
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <code className="text-sm font-mono">
                            {run.pipeline_id?.slice(-12) || run.id?.slice(0, 12)}
                          </code>
                        ...
                        <div className="flex items-center gap-4 text-xs text-muted-foreground">
                          ...
                          {topicStr && (
                            <span className="truncate max-w-[300px]">{topicStr.slice(0, 60)}</span>
                          )}
                          <span className="flex items-center gap-1">
                            <DollarSign className="h-3 w-3" />${(run.total_cost || 0).toFixed(4)}
                          </span>
                          {run.total_duration_ms ? (
                            <span>{(run.total_duration_ms / 1000).toFixed(0)}s</span>
                          ) : null}
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {(run.status === 'failed' || run.status === 'paused_cost') && (
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              resumeFromStage(run, run.last_completed_stage ?? 0);
                            }}
                          >
                            <SkipForward className="h-3 w-3" /> Resume
                          </Button>
                    ...
                    {isExpanded && (
                      <div className="mt-4 space-y-2">
                        <p className="text-xs font-medium text-muted-foreground mb-2">
                          Pipeline Stages — click stage bị lỗi để resume từ đó:
                        </p>
                        <div className="space-y-1">
                          {stages.map((stage, idx) => {
                            ...
                            return (
                              <div
                                ...
                                onClick={() => {
                                  if (isFailed) resumeFromStage(run, idx);
                                }}
                              >
                                ...
                                <span className="font-medium flex-1">
                                  {stage.name || agent?.name || `Stage ${idx + 1}`}
                                </span>
                                {stage.duration_ms ? (
                                ...
                                {isFailed && (
                                  <div className="flex items-center gap-2">
                                    ...
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      className="h-6 gap-1 text-[10px] border-red-500/50"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        resumeFromStage(run, idx);
                                      }}
                                    >
