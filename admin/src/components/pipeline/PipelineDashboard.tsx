...
export function PipelineDashboard() {
  ...
  const triggerPipeline = async () => {
    ...
    setTriggering(true);
    try {
      const body = triggerType === 'url' ? { videoUrl: triggerInput } : { topic: triggerInput };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">YouTube Agent Crew — 7-agent AI video factory</p>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        ...
                        {run.input?.topic && (
                          <span>Topic: {String(run.input.topic).substring(0, 50)}</span>
                        )}
                        {run.input?.videoUrl && (
                          <span>URL: {String(run.input.videoUrl).substring(0, 50)}</span>
                        )}
                        {run.stages?.length > 0 && (
                          <span>
                    ...
                    {(run.status === 'failed' || run.status === 'paused_cost') && (
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => resumePipeline(run.pipelineId || run.id)}
                      >
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-3 text-xs text-muted-foreground">
                        <Badge variant="secondary">
                          Stage {cp.stageIndex + 1}: {cp.stageName}
                        </Badge>
                        {cp.checkpointedAt && (
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => resumePipeline(cp.pipelineId)}
                    >
