...
const BG_PROMPTS = [
  {
    label: 'Cavern Mystery',
    prompt:
      'A vast dimly lit cavern with a single figure silhouetted, gazing into a glowing abyss. Bioluminescent particles, cinematic lighting, dark moody atmosphere',
  },
  {
    label: 'City Night',
    prompt:
      'Aerial view of a futuristic city at night with neon lights and rain, cyberpunk atmosphere, dark blue and purple tones, cinematic',
  },
  {
    label: 'Mountain Dawn',
    prompt:
      'Person standing on mountain peak at golden hour, dramatic clouds below, warm orange and gold tones, inspirational, cinematic wide shot',
  },
  {
    label: 'Abstract Dark',
    prompt:
      'Abstract dark background with flowing gold particles and light trails, deep navy blue, minimalist, elegant, perfect for podcast overlay',
  },
  {
    label: 'Library Wisdom',
    prompt:
      'Grand ancient library with towering bookshelves, warm candlelight, dust particles in light beams, atmospheric, knowledge and wisdom',
  },
  {
    label: 'Space Nebula',
    prompt:
      'Deep space nebula with vibrant purple and blue colors, stars, cosmic dust, dark background, ethereal and vast',
  },
  {
    label: 'Studio Mic',
    prompt:
      'Professional podcast studio with a single condenser microphone in dramatic spotlight, dark background, bokeh lights, intimate recording atmosphere',
  },
  {
    label: 'Coffee Think',
    prompt:
      'Steam rising from a coffee cup on a desk near a rainy window at night, warm indoor lighting, contemplative mood, cinematic close-up',
  },
];
    ...
    try {
      ...
      const res = await fetch('/api/youtube-crew/generate-image', {
        ...
        body: JSON.stringify({
          prompt,
          type,
          width: type === 'thumbnail' ? 1280 : 1920,
          height: type === 'thumbnail' ? 720 : 1080,
        }),
      });
      if (res.ok) {
    ...
    <div className="space-y-4">
      ...
      <Tabs value={studioTab} onValueChange={setStudioTab}>
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="thumbnail" className="gap-1">
        ...
        <TabsContent value="templates" className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              Click = apply. Star = set as default for all new runs.
            </p>
            <div className="flex border rounded-md overflow-hidden">
              {(['all', 'podcast', 'general'] as const).map((f) => (
                <button
                  key={f}
                  className={`px-3 py-1.5 text-xs ${templateFilter === f ? 'bg-primary text-primary-foreground' : 'bg-muted hover:bg-muted/80'}`}
          ...
          <div className="grid grid-cols-3 gap-3">
            {filteredTemplates.map((tpl) => {
              ...
              return (
                ...
                >
                  <div
                    className="h-36 rounded-t-lg relative overflow-hidden"
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <p
                        className="text-[10px] mt-1 px-2 py-0.5 rounded bg-black/30"
                        style={{ color: tpl.config.accent_color }}
                      >
                    ...
                    {tpl.category === 'podcast' && (
                      <Badge className="absolute bottom-2 left-2 text-[9px] h-4 bg-blue-600/80">
                        PODCAST
                      </Badge>
                    )}
                  </div>
                  <CardContent className="p-3">
                    <div className="flex items-center gap-2">
                      ...
                      {isDefault && (
                        <Badge
                          variant="outline"
                          className="text-[9px] h-4 text-yellow-600 border-yellow-600"
                        >
                          DEFAULT
                        </Badge>
                      )}
        ...
        <TabsContent value="thumbnail" className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
              ...
              <Card>
                <CardHeader className="pb-2">
                  ...
                  <CardDescription className="text-xs">
                    1280x720 — ảnh đầu tiên viewer thấy
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div
                    ...
                    style={{
                      background: thumbnailUrl
                        ...
                        : videoConfig.bg_color,
                    }}
                  >
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40">
                      <p
                        className="text-2xl font-black text-white drop-shadow-lg text-center px-4 leading-tight"
                        style={{
                          color: videoConfig.text_color,
                      ...
                      <p
                        className="text-xs mt-2 font-bold px-3 py-1 rounded"
            ...
            <div className="space-y-3">
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Edit Thumbnail</CardTitle>
                </CardHeader>
        ...
        <TabsContent value="background" className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div
                    className="aspect-video rounded-lg border overflow-hidden relative"
                    style={{
                      background: bgUrl ? `url(${bgUrl}) center/cover` : videoConfig.bg_color,
                    }}
                  >
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <div className="text-center">
                        <p className="text-lg font-bold" style={{ color: videoConfig.text_color }}>
                          {videoConfig.channel_name}
                        </p>
                        {/* Waveform visualizer */}
                        ...
                        <p
                          className="mt-4 text-sm px-8 leading-relaxed"
                          style={{
            ...
            <div className="space-y-3">
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm">Background Source</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div>
                    <Label className="text-xs">Image Source</Label>
                    <select
                      className="w-full mt-1 text-sm border rounded-md p-2 bg-background"
                      value={videoConfig.image_source}
                      onChange={(e) =>
                        setVideoConfig((v) => ({
                          ...v,
                          image_source: e.target.value as VideoConfig['image_source'],
                        }))
                      }
                    >
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="flex-1 gap-1"
                      disabled={generating}
                      onClick={() => generateImage('background')}
                    >
                      {generating ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
        ...
        <TabsContent value="layout" className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setVideoConfig((v) => ({ ...v, width: Number(e.target.value) }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setVideoConfig((v) => ({ ...v, height: Number(e.target.value) }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setVideoConfig((v) => ({ ...v, fps: Number(e.target.value) }))
                      }
                    />
                  </div>
                </div>
                <div>
                  ...
                  <Input
                    className="mt-1 text-sm"
                    value={videoConfig.channel_name}
                    onChange={(e) =>
                      setVideoConfig((v) => ({ ...v, channel_name: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <Slider
                    ...
                    max={30}
                    step={1}
                    value={[videoConfig.crf]}
                    onValueChange={(v) => setVideoConfig((c) => ({ ...c, crf: v[0] }))}
                  />
                  <p className="text-[10px] text-muted-foreground mt-1">
                    {videoConfig.crf <= 18
                      ? 'Very High quality — large file'
                      : videoConfig.crf <= 23
                        ? 'Good balance — recommended'
                        : 'Compressed — fast upload'}
                  </p>
                </div>
                <div>
                  ...
                  <Slider
                    ...
                    max={120}
                    step={5}
                    value={[videoConfig.comfyui_timeout]}
                    onValueChange={(v) => setVideoConfig((c) => ({ ...c, comfyui_timeout: v[0] }))}
                  />
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">Colors & Subtitles</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="grid grid-cols-3 gap-2">
                  {(
                    ...
                    <div key={c.key}>
                      ...
                      <div className="flex gap-1 mt-1">
                        <input
                          type="color"
                          value={videoConfig[c.key]}
                          onChange={(e) =>
                            setVideoConfig((v) => ({ ...v, [c.key]: e.target.value }))
                          }
                ...
                <div>
                  <Label className="text-xs">
                    Subtitle Size: {videoConfig.subtitle_font_size}px
                  </Label>
                  <Slider
                    className="mt-2"
                    min={24}
                    ...
                    onValueChange={(v) =>
                      setVideoConfig((c) => ({ ...c, subtitle_font_size: v[0] }))
                    }
                  />
                </div>
                <div>
                  <Label className="text-xs">
                    Subtitle Bottom: {videoConfig.subtitle_margin_bottom}px
                  </Label>
                  <Slider
                    className="mt-2"
                    min={20}
                    ...
                    onValueChange={(v) =>
                      setVideoConfig((c) => ({ ...c, subtitle_margin_bottom: v[0] }))
                    }
                  />
                </div>
                ...
                <div
                  ...
                  style={{ background: videoConfig.bg_color, minHeight: 120 }}
                >
                  <p
                    className="text-center text-sm font-bold"
                    style={{ color: videoConfig.text_color }}
                  >
                    {videoConfig.channel_name}
                  </p>
                  ...
                  >
                    <span
                      className="px-3 py-1 rounded bg-black/60"
                      style={{
                        color: '#fff',
                        fontSize: `${Math.max(10, videoConfig.subtitle_font_size * 0.3)}px`,
                      }}
