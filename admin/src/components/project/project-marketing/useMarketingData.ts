...
export function useMarketingData(projectId: string, projectName: string, projectSlug: string) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (campaignsError) {
        console.warn('Marketing campaigns query error:', campaignsError.message);
      ...
      try {
        ...
        if (postsResult.error) {
          console.warn('Content queue query error:', postsResult.error.message);
          ...
          const transformedPosts = postsResult.data.map((item) => ({
            ...
            campaign_name: item.metadata?.campaign_name || item.title,
          }));
          setScheduledPosts(transformedPosts);
          setStats((prev) => ({ ...prev, scheduledPosts: transformedPosts.length }));
        }
      } catch (postsErr) {
        console.warn('Content queue query error:', postsErr);
      }
    } catch {
      ...
      setCampaigns([
        {
          id: '1',
          name: `${projectName} Launch Campaign`,
          type: 'social_media',
          status: 'running',
          platforms: ['facebook', 'linkedin', 'instagram'],
          content: `🚀 Excited to announce ${projectName}! Stay tuned for more updates.`,
          scheduled_at: null,
        ...
        },
        {
          ...
          content: 'Tips & Tricks weekly series for our community.',
  ...
  const loadMarketingDocs = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error loading marketing docs:', error);
      setMarketingDocs(null);
    } finally {
  ...
  useEffect(() => {
    if (activeTab === 'docs' && !marketingDocs) loadMarketingDocs();
  }, [activeTab, marketingDocs, loadMarketingDocs]);
  ...
  const handleCreateMarketingPack = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/marketing-docs/${projectSlug}/create`, {
        ...
        body: JSON.stringify({
          name: newPackInfo.name || projectName,
          category: newPackInfo.category,
          oneLiner: newPackInfo.oneLiner,
          targetMarket: newPackInfo.targetMarket,
        }),
      });
      const data = await response.json();
      if (data.success) {
        toast.success('Đã tạo Marketing Pack thành công!');
        setShowCreatePackDialog(false);
        setNewPackInfo({ name: '', category: '', oneLiner: '', targetMarket: 'Vietnam' });
        loadMarketingDocs();
      } else {
        toast.error(data.error || 'Không thể tạo Marketing Pack');
      }
    } catch (error) {
      console.error('Error creating marketing pack:', error);
      toast.error('Lỗi khi tạo Marketing Pack');
    } finally {
  ...
  const handleDeleteCampaign = useCallback(
    async (campaignId: string) => {
      if (
        !confirm('Bạn có chắc muốn xóa chiến dịch này? Tất cả bài đăng liên quan cũng sẽ bị xóa.')
      )
        return;
      try {
        ...
        if (result.success) {
          ...
          toast.error(result.error || 'Không thể xóa chiến dịch');
        }
      } catch (error) {
    ...
    async (campaignId: string) => {
      try {
        ...
        if (result.success) {
          ...
          toast.error(result.error || 'Không thể nhân bản chiến dịch');
        }
      } catch (error) {
        console.error('Duplicate campaign error:', error);
        toast.error('Lỗi khi nhân bản chiến dịch');
      }
    },
    [loadData]
  );
  ...
  const handleStatusChange = useCallback(
    async (campaignId: string, newStatus: 'running' | 'paused' | 'completed') => {
      try {
        const result = await marketingApi.updateCampaignStatus(campaignId, newStatus);
        if (result.success) {
          const labels = {
            running: 'Đã bắt đầu chạy',
            paused: 'Đã tạm dừng',
  ...
  const handleGeneratePosts = useCallback(
    async (campaign: Campaign) => {
      ...
      setAiContent('');
      try {
        const result = await marketingApi.generateAndSaveCampaignPosts(campaign.id, {
        ...
        if (result.success) {
          if (result.saved) {
            toast.success(`Đã tạo ${result.posts?.length || 0} bài đăng từ AI`);
            loadData();
          } else {
            setAiContent(result.aiResponse);
            toast.info('AI đã tạo content, cần xử lý thủ công');
          }
        } else {
          toast.error(result.error || 'Không thể tạo bài đăng');
        }
      } catch (error) {
        console.error('Generate posts error:', error);
        toast.error('Lỗi khi tạo bài đăng với AI');
      } finally {
    ...
    try {
      const { error } = await supabaseAdmin
        ...
        .update({
          ...
          scheduled_for: new Date(editingPost.scheduled_at).toISOString(),
        })
    ...
    async (postId: string) => {
      ...
      try {
        const { error } = await supabaseAdmin
          ...
          .eq('id', postId);
        if (error) throw error;
        toast.success('Bài đăng sẽ được xử lý ngay');
        loadData();
      } catch (error) {
  ...
  const handleAIWritePost = useCallback(async () => {
    ...
    try {
      const result = await marketingApi.quickGeneratePost({
        platform: selectedPlatforms[0],
        topic: quickPostContent || `Giới thiệu về ${projectName}`,
        projectSlug,
        tone: 'professional',
      });
      if (result.success) {
        setQuickPostContent(result.content);
        toast.success('AI đã viết bài cho bạn!');
      } else {
        toast.error(result.error || 'AI không thể viết bài');
      }
    } catch (error) {
  ...
  const handleQuickPost = useCallback(async () => {
    ...
    try {
      const campaignData = {
        name: `Quick Post - ${new Date().toLocaleDateString('vi-VN')}`,
        type: 'social_media',
      ...
      let result = await supabaseAdmin
        .from('marketing_campaigns')
        .insert({ project_id: projectId, ...campaignData })
        .select()
        .single();
      ...
      toast.success(
        isScheduled
          ? `Đã lên lịch đăng lên ${selectedPlatforms.length} nền tảng`
          : `Đã đăng lên ${selectedPlatforms.length} nền tảng`
      );
      setQuickPostContent('');
  ...
  const handleCreateCampaign = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Không thể tạo chiến dịch. Vui lòng kiểm tra database schema.');
    }
