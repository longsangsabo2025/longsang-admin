...
export function AIPlatformWriterConfigDialog({
  ...
  const [writers, setWriters] =
    useState<Record<PlatformType, AIPlatformWriterConfig>>(PLATFORM_WRITER_DEFAULTS);
  ...
  const [customPrompts, setCustomPrompts] = useState<Record<PlatformType, string>>(
    {} as Record<PlatformType, string>
  );
  const [useCustomPrompts, setUseCustomPrompts] = useState<Record<PlatformType, boolean>>(
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh]">
        <DialogHeader>
          ...
          <DialogDescription>Cấu hình AI viết content riêng cho từng nền tảng</DialogDescription>
        </DialogHeader>
                  ...
                  <TabsContent value="prompt" className="space-y-4 mt-4">
                    <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                      <p className="text-sm text-blue-300">
                        💡 <strong>Prompt Preview:</strong> Xem và chỉnh sửa prompt cho{' '}
                        <span className="capitalize font-bold">{selectedPlatform}</span>. Có thể
                        paste prompt từ nơi khác hoặc test trực tiếp.
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(v) =>
                          setUseCustomPrompts((prev) => ({ ...prev, [selectedPlatform]: v }))
                        }
                      />
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <Button variant="secondary" size="sm" onClick={testPrompt} disabled={testing}>
                        {testing ? (
