...
import { MarketingMasterplan, MasterplanCampaign, PlatformType } from '@/types/ai-marketing';
...
export function MarketingMasterplanView({
  ...
  const totalCampaigns =
    masterplan.phases?.reduce((sum, phase) => sum + (phase.campaigns?.length || 0), 0) || 0;

  const totalPosts =
    masterplan.phases?.reduce(
      (sum, phase) =>
        sum +
        (phase.campaigns?.reduce(
          (cSum, camp) =>
            cSum +
            (camp.contentCalendar?.reduce((wSum, week) => wSum + (week.posts?.length || 0), 0) ||
              0),
          0
        ) || 0),
      0
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-purple-200 bg-purple-50/50 dark:bg-purple-950/20">
        ...
        <CardContent>
          <p className="text-muted-foreground leading-relaxed">{masterplan.executiveSummary}</p>
                    ...
                    return (
                      <div key={category}>
                        ...
                        <ul className="space-y-2">
                          {items.map((rec, i) => (
                            <li key={i} className="p-2 rounded border bg-muted/30 text-sm">
                            ...
                            <CardContent className="p-4">
                              <div className="flex items-start justify-between">
                                ...
                                <div className="flex flex-col gap-2 ml-4">
                                  ...
                                  {onGenerateContent && (
                                    <Button size="sm" onClick={() => onGenerateContent(campaign)}>
                  ...
                  {masterplan.overallKPIs.map((kpi, index) => (
                    ...
                    >
                      ...
                      <div className="flex items-baseline gap-2 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground">from {kpi.baseline}</span>
                      </div>
