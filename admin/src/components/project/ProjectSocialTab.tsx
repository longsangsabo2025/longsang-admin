...
const socialLinksTable = () => supabaseAdmin.from('project_social_links') as any;
...
export function ProjectSocialTab({ projectId, projectName }: Readonly<ProjectSocialTabProps>) {
  ...
  const saveSocialLink = async () => {
    ...
    try {
      ...
      if (editingLink) {
        ...
        // Create new
        const { error } = await socialLinksTable().insert({
          ...saveData,
  ...
  const deleteSocialLink = async (id: string) => {
    ...
    try {
      const { error } = await socialLinksTable().delete().eq('id', id);
          ...
          >
            ...
            <DialogContent className="max-w-md">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        follower_count: Number.parseInt(e.target.value, 10) || 0,
                      })
                    }
                  />
                </div>
                ...
                {credentials.length > 0 && (
                  <div className="space-y-2 pt-2 border-t">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        setFormData({ ...formData, credential_id: v === 'none' ? '' : v })
                      }
                    >
                      <SelectTrigger>
                ...
                {formData.credential_id && (
                  <div className="flex items-center justify-between pt-2 border-t">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setFormData({ ...formData, auto_post_enabled: checked })
                      }
                    />
                  </div>
                )}
              ...
              <DialogFooter>
                ...
                <Button onClick={saveSocialLink}>{editingLink ? 'Cập nhật' : 'Thêm'}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t">
                    <div className="flex items-center gap-1">
                      ...
                      >
                        <Star
                          className={`h-4 w-4 ${link.is_primary ? 'fill-yellow-500 text-yellow-500' : ''}`}
                        />
                      </Button>
                    </div>
                    <div className="flex items-center gap-1">
