...
import { AIStrategistConfig, DEFAULT_STRATEGIST_CONFIG } from '@/types/ai-marketing';
    ...
    try {
      ...
      if (data?.settings?.ai_config?.strategist) {
        const loadedConfig = {
          ...
          ...data.settings.ai_config.strategist,
        };
...
${buildUserPrompt()}`;
  ...
  const testPrompt = async () => {
    ...
    try {
      ...
      const testPromptWithSample = prompt.replace('[Tên dự án]', 'Test Project').replace(
        '[Nội dung tài liệu sẽ được đọc tự động từ database]',
        `=== Tài liệu mẫu ===
...
Mục tiêu: Tăng downloads và user retention`
      ...
      const systemMatch = testPromptWithSample.match(
            ...
            <TabsContent value="model" className="space-y-4 mt-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  {config.expertise.yearsExperience} năm kinh nghiệm
                </p>
              </div>
            </TabsContent>
            ...
            <TabsContent value="prompt" className="space-y-4 mt-4">
              <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-3">
                <p className="text-sm text-purple-300">
                  💡 <strong>Prompt Preview:</strong> Xem và chỉnh sửa prompt cuối cùng sẽ gửi cho
                  AI. Bạn có thể paste prompt từ nơi khác hoặc tùy chỉnh theo ý muốn.
                </p>
              </div>
              ...
              {testResult && (
                <div className="space-y-2">
                  ...
                  <div className="bg-green-950/30 border border-green-500/30 rounded-lg p-4 max-h-[200px] overflow-auto">
                    <pre className="text-sm whitespace-pre-wrap text-green-200">{testResult}</pre>
                  </div>
                </div>
              )}
            </TabsContent>
