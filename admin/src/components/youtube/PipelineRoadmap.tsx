  ...
  'dung-day-di': [
    {
      ...
      prompt:
        'Viết như một triết gia bóng tối thức tỉnh giữa đêm. Giọng nói đầy uy lực, mỗi câu như một cú đấm vào tâm trí. Dùng ẩn dụ mạnh mẽ về chiến binh, bóng tối, sự thật phũ phàng. Kết thúc bằng lời kêu gọi hành động khiến người xem không thể ngồi yên.',
    },
    {
      label: '⚔️ Chiến binh kỷ luật',
      tone: 'dark-philosophical',
      prompt:
        'Tập trung vào kỷ luật bản thân như vũ khí tối thượng. Dùng ngôn ngữ quân sự, kỹ năng sống. Cấu trúc: mở bằng sự thật phũ phàng → phân tích tâm lý → chiến thuật cụ thể → kết bằng thử thách cho người xem.',
    },
    {
      label: '🧠 Dark psychology',
      tone: 'dark-philosophical',
      prompt:
        'Phân tích tâm lý tối (dark psychology) một cách sắc bén. Chỉ ra cách người ta bị thao túng hàng ngày, dùng ví dụ thực tế. Giọng điệu bình tĩnh nhưng khiến người xem giật mình. Kết thúc bằng cách phòng thủ.',
    },
  ],
  'sach-15-phut': [
    {
      ...
      tone: 'storytelling',
      prompt:
        'Tóm tắt sách theo framework PACER: Problem → Analysis → Core Ideas → Examples → Review. Giọng thân mật như mentor chia sẻ ở quán cà phê. Mỗi ý chính kèm 1 ví dụ thực tế áp dụng được ngay.',
    },
    {
      label: '🎯 3 bài học thay đổi đời',
      tone: 'storytelling',
      prompt:
        'Chọn 3 bài học quan trọng nhất từ sách. Kể theo dạng storytelling, mỗi bài học = 1 câu chuyện mini. Dùng ngôn ngữ gần gũi, dễ hiểu, tránh thuật ngữ hàn lâm. Kết thúc: "Nếu bạn chỉ nhớ 1 điều..."',
    },
    {
      label: '⚡ So sánh 2 cuốn sách',
      tone: 'storytelling',
      prompt:
        'So sánh 2 cuốn sách cùng chủ đề. Cấu trúc: giới thiệu cả 2 → so sánh triết lý → điểm mạnh/yếu → ai nên đọc cuốn nào. Giọng công bằng, analytical nhưng vẫn dễ nghe.',
    },
  ],
  'ai-builder-vn': [
    {
      ...
      tone: 'educational',
      prompt:
        'Hướng dẫn chi tiết step-by-step cho người mới. Từ setup → cấu hình → build → deploy. Mỗi bước kèm screenshot/demo description. Tone: tech builder thực tế, nói thẳng khó ở đâu, mẹo gì hay.',
    },
    {
      label: '⚡ Automation workflow',
      tone: 'educational',
      prompt:
        'Show một automation workflow hoàn chỉnh: vấn đề → giải pháp → tools dùng → cách kết nối → kết quả. Dùng ngôn ngữ "build in public", chia sẻ cả failures. Kết: cost estimate + time saved.',
    },
    {
      label: '🔧 Tool review thực chiến',
      tone: 'educational',
      prompt:
        'Review tool AI một cách thật thà: pros/cons/pricing/alternatives. Test thật, show kết quả thật. So sánh với tools tương tự. Kết luận: dùng khi nào, ai nên dùng, ai không.',
    },
  ],
  'tien-thong-minh': [
    {
      ...
      tone: 'dark-philosophical',
      prompt:
        'Phân tích chủ đề tài chính với data cụ thể. Dùng số liệu Việt Nam thực tế. Giọng financial mentor thẳng thắn: "Sự thật là..." Cấu trúc: sai lầm phổ biến → data chứng minh → chiến lược → action items.',
    },
    {
      label: '⚠️ Bẫy tài chính',
      tone: 'dark-philosophical',
      prompt:
        'Vạch trần một bẫy tài chính phổ biến. Bắt đầu bằng scenario ai cũng gặp → vén màn cơ chế hoạt động → cách ngân hàng/công ty kiếm tiền từ bạn → cách thoát bẫy. Tone mạnh, có backing data.',
    },
    {
      label: '📈 Hướng dẫn đầu tư',
      tone: 'educational',
      prompt:
        'Hướng dẫn đầu tư cho người mới, tránh quá phức tạp. Dùng ví dụ với số tiền thực (5tr, 10tr, 50tr). So sánh các kênh đầu tư. Honest về risk. Kết: "Bước đầu tiên bạn nên làm ngay..."',
    },
  ],
  'ly-black': [
    {
      ...
      tone: 'dark-philosophical',
      prompt:
        'Write as Ly Black — an AI virtual influencer with a poetic, mysterious voice. Short philosophical reflections on existence, consciousness, art. Bilingual (Vietnamese/English). Visual and metaphorical language. Each piece feels like a digital diary entry.',
    },
    {
      label: '🤖 AI & Humanity',
      tone: 'dark-philosophical',
      prompt:
        'Explore the boundary between AI and humanity through Ly Black\'s perspective. Thought-provoking questions, not answers. Artistic language, noir aesthetic. "What if the most human thing about AI is its doubt?"',
    },
  ],
};
...
const DEFAULT_PRESETS: { label: string; prompt: string; tone: string }[] = [
  {
    ...
    tone: 'storytelling',
    prompt:
      'Viết rõ ràng, dễ hiểu, cuốn hút. Dùng ví dụ thực tế. Giọng điệu tự nhiên như đang kể chuyện. Tránh thuật ngữ khó hiểu.',
  },
  {
    label: '🎯 Deep analysis',
    tone: 'educational',
    prompt:
      'Phân tích chuyên sâu với dẫn chứng. Cấu trúc logic: vấn đề → nguyên nhân → giải pháp → kết luận. Tone nghiêm túc nhưng accessible.',
  },
];
...
const STEP_LOG_PATTERNS: Record<keyof PipelineConfig, RegExp> = {
  scriptWriter:
    /script|writing|gemini|gpt|claude|model|prompt|topic|transcript|words?\s*count|generating/i,
  storyboard: /storyboard|scene|visual|hailuo|prompt.*gen|motion|transition/i,
  imageGen:
    /image|flux|dall-e|midjourney|render.*image|generating.*image|scene.*done|scene.*generat|scene.*fail/i,
...
function deriveStepStatuses(
  ...
  if (unclassified.length > 0) {
    const target =
      steps.find((k) => config[k].enabled && result[k].logs.length === 0) || 'scriptWriter';
    result[target].logs.push(...unclassified);
  }

  // Derive statuses for enabled steps
  const enabledSteps = steps.filter(
    (k) => config[k].enabled && STEPS.find((s) => s.key === k)?.status !== 'coming-soon'
  );
...
function extractScriptResult(
  run?: ActiveRunInfo & { result?: { files?: Record<string, unknown> } }
): {
  title?: string;
  script?: string;
  wordCount?: number;
  ...
  const meta = result.files['script.json'] as
    | { title?: string; wordCount?: number; cost?: number; model?: string }
    | undefined;
...
function extractStoryboardResult(
  run?: ActiveRunInfo & { result?: { files?: Record<string, unknown> } }
): {
  scenes?: {
    scene: number;
  ...
  const result = (run as { result?: { files?: Record<string, unknown> } }).result;
  if (!result?.files) return null;
  const sbJson = result.files['storyboard.json'] as
    | {
        scenes?: {
          scene: number;
          dialogue: string;
...
function extractImageGenResult(
  run?: ActiveRunInfo & { result?: { files?: Record<string, unknown> } }
): {
...
function extractVoiceoverResult(
  run?: ActiveRunInfo & { result?: { files?: Record<string, unknown> } }
): {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2 text-[10px] text-muted-foreground">
            <Badge variant="outline" className="text-[10px]">
              {imgResult.successCount}/{imgResult.totalScenes} scenes
            </Badge>
            {imgResult.failCount > 0 && (
              <Badge variant="destructive" className="text-[10px]">
                {imgResult.failCount} failed
              </Badge>
            )}
          </div>
        </div>
    ...
    <div className="mt-3 pt-3 border-t">
      <div className="rounded bg-green-950/30 border border-green-500/20 p-3 space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2 text-[10px] text-muted-foreground">
            <Badge variant="outline" className="text-[10px]">
              {voResult.successCount}/{voResult.totalClips} clips
            </Badge>
            <Badge variant="outline" className="text-[10px]">
              ~{voResult.totalDuration}s
            </Badge>
            {voResult.failCount > 0 && (
              <Badge variant="destructive" className="text-[10px]">
                {voResult.failCount} failed
              </Badge>
            )}
          </div>
        </div>
            ...
            {showClips && (
              <div className="space-y-1.5">
                {displayClips.map((clip) => (
                  <div
                    key={clip.scene}
                    className="flex items-center gap-2 bg-background/30 rounded px-2 py-1"
                  >
...
export default function PipelineRoadmap({
  ...
  activeRun,
  onVoiceConfigChange,
}: PipelineRoadmapProps) {
  const [config, setConfig] = useState<PipelineConfig>(() =>
    loadSavedConfig(channelId, channelStyle)
  );
  ...
  useEffect(() => {
    if (activeRun?.status === 'running') {
      const runningStep = (
        Object.entries(stepStatuses) as [keyof PipelineConfig, { status: StepStatus }][]
  ...
  const updateStep = <K extends keyof PipelineConfig>(
    ...
    if (step === 'voiceover' && onVoiceConfigChange) {
      ...
      if (u.engine !== undefined || u.voice !== undefined || u.speed !== undefined) {
        onVoiceConfigChange({ engine: u.engine, voice: u.voice, speed: u.speed } as Partial<{
          engine: string;
          voice: string;
          speed: number;
        }>);
  ...
  const totalEstimate = STEPS.filter((s) => config[s.key].enabled)
    .map((s) => s.estimate)
    .join(' + ');
    ...
    <div className="space-y-3">
                  ...
                  className={cn(
                    ...
                    !isStepRunning &&
                      ...
                      ? 'border-l-2 border-l-current ' +
                          step.color.split(' ')[0].replace('text-', 'border-l-')
                      : '',
                  ...
                  <CardContent className="p-3">
                            ...
                            : isStepDone
                              ...
                              : isStepFailed
                                ...
                                : isStepInterrupted
                                  ...
                                  : stepConfig.enabled && !isComingSoon
                                    ? step.color
                      ...
                      >
                        ...
                        {stepConfig.enabled && !isComingSoon && step.getModel && (
                          <div className="flex items-center gap-1.5 mt-0.5">
                            ...
                            <Badge
                              variant="outline"
                              className="text-[9px] px-1 py-0 font-mono border-dashed"
                            >
                              ...
                              <div>
                                <Switch
                                  ...
                                  onCheckedChange={(checked) =>
                                    updateStep(step.key, { enabled: checked } as Partial<
                                      PipelineConfig[keyof PipelineConfig]
                                    >)
                                  }
                                />
                              </div>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (!stepConfig.enabled) {
                                updateStep(step.key, { enabled: true } as Partial<
                                  PipelineConfig[keyof PipelineConfig]
                                >);
                              }
                              onRunStep(step.key, {
                          ...
                          <div className="mt-3 pt-3 border-t space-y-1">
                            <div className="flex items-center justify-between text-[11px]">
                              ...
                              <span className="text-muted-foreground">
                                {Math.round(
                                  (Date.now() -
                                    new Date(activeRun?.logs?.[0]?.t || Date.now()).getTime()) /
                                    1000
                                )}
                                s elapsed
                              </span>
                    ...
                    {hasLogs && (
                      <div
                        className={cn(
                          'mt-3 pt-3 border-t',
                          !isStepRunning && extractProgress(stepStatus.logs) > 0 && 'mt-2 pt-2'
                        )}
                      >
                            ...
                            <div className="rounded bg-green-950/30 border border-green-500/20 p-3 space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-2 text-[10px] text-muted-foreground">
                                  ...
                                  {scriptResult.cost != null && (
                                    <Badge variant="outline" className="text-[10px]">
                                      ${scriptResult.cost.toFixed(4)}
                                    </Badge>
                                  )}
                                </div>
                              </div>
                              ...
                              {scriptResult.script && (
                                <ScrollArea className="max-h-40">
                                  <pre className="text-[11px] text-muted-foreground whitespace-pre-wrap leading-relaxed">
                                    {scriptResult.script.slice(0, 1500)}
                                    {scriptResult.script.length > 1500 ? '\n\n... (truncated)' : ''}
                                  </pre>
                                </ScrollArea>
                              )}
                            </div>
                            ...
                            <div className="rounded bg-purple-950/30 border border-purple-500/20 p-3 space-y-2">
                                        ...
                                        <div key={s.scene} className="text-[11px] space-y-1">
                                          <div className="flex items-center gap-1.5">
                                            ...
                                            <span className="text-muted-foreground truncate">
                                              {s.dialogue?.slice(0, 80)}
                                              {s.dialogue?.length > 80 ? '...' : ''}
                                            </span>
                                            {imgResult && !imgUrl && (
                                              <Badge
                                                variant="destructive"
                                          ...
                                          <p className="text-purple-300/70 pl-12 truncate text-[10px]">
                              ...
                              {!sbResult.scenes && sbResult.promptsTxt && (
                                <ScrollArea className="max-h-32">
                                  <pre className="text-[11px] text-muted-foreground whitespace-pre-wrap leading-relaxed">
                                    {sbResult.promptsTxt.slice(0, 800)}
                                    {sbResult.promptsTxt.length > 800 ? '\n\n... (truncated)' : ''}
                                  </pre>
                                </ScrollArea>
                              )}
                            </div>
      ...
      {activeRun?.status === 'interrupted' && onResume && (
        <Button
          ...
          onClick={onResume}
        >
          <RotateCcw className="h-4 w-4 mr-2" /> Resume Pipeline (
          {activeRun.completedSteps?.length || 0}/{activeRun.pipelineSteps?.length || '?'} steps
          done)
        </Button>
      )}
      ...
      >
        {isRunning ? (
          <>
            ...
            {parallelCount > 1 ? ` (${parallelCount} active)` : ''}...
          </>
        ) : batchCount > 0 ? (
          <>
            <Layers className="h-4 w-4 mr-2" /> Run {batchCount} Topics in Parallel ({enabledCount}{' '}
            steps each)
          </>
        ) : (
          <>
    ...
    >
      {visibleLogs.map((log, i) => (
        <div key={i} className="flex items-start gap-1.5">
          {levelIcon(log.level)}
          <span className={cn('break-all', levelColor(log.level))}>{log.msg}</span>
        </div>
      ))}
      {isRunning && (
...
function StepConfig({
  ...
  switch (stepKey) {
    case 'scriptWriter':
      return (
        <ScriptWriterConfig
          config={config.scriptWriter}
          channelId={channelId}
          onUpdate={(u) => onUpdate('scriptWriter', u)}
        />
      );
    case 'storyboard':
      return (
        <StoryboardConfig
          ...
          channelId={channelId}
          wordTarget={config.scriptWriter.wordTarget}
          onUpdate={(u) => onUpdate('storyboard', u)}
        />
      );
    case 'imageGen':
      return (
        <ImageGenConfig
          ...
          activeRun={activeRun}
          onUpdate={(u) => onUpdate('imageGen', u)}
        />
      );
    case 'voiceover':
      return (
        <VoiceoverConfig
          ...
          activeRun={activeRun}
          onUpdate={(u) => onUpdate('voiceover', u)}
        />
      );
    case 'assembly':
      return (
        <AssemblyConfig
          ...
          activeRun={activeRun}
          onUpdate={(u) => onUpdate('assembly', u)}
        />
      );
    default:
      return null;
    ...
    <div className="grid grid-cols-2 gap-3">
      ...
      <div className="space-y-1">
        ...
        <Input
          ...
          onChange={(e) => onUpdate({ wordTarget: parseInt(e.target.value, 10) || 2500 })}
        />
        <p className="text-[10px] text-muted-foreground">
          ≈ {Math.round(config.wordTarget / 250)} phút video (
          {Math.round((config.wordTarget / 250) * 60)}s audio)
        </p>
      </div>
...
function StoryboardConfig({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Smart Scene Calculator ── */}
      {(() => {
        const estAudioSec = Math.round((wordTarget / 250) * 60);
        const estMinutes = Math.round(wordTarget / 250);
        const recommendedScenes = Math.max(8, Math.round(estAudioSec / config.duration));
        const currentCoverage = config.scenes * config.duration;
        const mismatch = Math.abs(currentCoverage - estAudioSec) > estAudioSec * 0.2;
        const scenesMatch = config.scenes === recommendedScenes;
        return (
          <div
            className={cn(
          ...
          >
            <div className="flex items-center justify-between">
              ...
              {!scenesMatch && (
                <Badge
                  variant="secondary"
                  className="text-[10px] px-2 py-0.5 cursor-pointer hover:bg-primary/20 transition-colors"
                  onClick={() => onUpdate({ scenes: recommendedScenes })}
                >
                  🎯 Auto: {recommendedScenes} scenes
                </Badge>
              )}
            </div>
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                <p className="text-[18px] font-bold tabular-nums">{estMinutes}</p>
                <p className="text-[9px] text-muted-foreground">phút audio</p>
                <p className="text-[9px] text-muted-foreground/60">{wordTarget} từ</p>
              </div>
              <div>
                <p className="text-[18px] font-bold tabular-nums">{config.duration}s</p>
                <p className="text-[9px] text-muted-foreground">/ ảnh</p>
                <p className="text-[9px] text-muted-foreground/60">chuyển cảnh</p>
              </div>
              <div>
                <p
                  className={cn(
                    'text-[18px] font-bold tabular-nums',
                    mismatch ? 'text-yellow-400' : 'text-green-400'
                  )}
                >
                  {config.scenes}
                </p>
                <p className="text-[9px] text-muted-foreground">scenes</p>
                <p className="text-[9px] text-muted-foreground/60">= {config.scenes} ảnh</p>
              </div>
            </div>
            {mismatch && (
              <p className="text-[10px] text-yellow-400">
                ⚡ {config.scenes} scenes × {config.duration}s = {currentCoverage}s ảnh — nhưng
                audio ~{estAudioSec}s.
                {config.scenes < recommendedScenes
                  ? ` Cần thêm ${recommendedScenes - config.scenes} scenes (→ ${recommendedScenes}) để mỗi ảnh đổi mỗi ${config.duration}s.`
                  : ` Có thể giảm xuống ${recommendedScenes} scenes.`}
              </p>
            )}
          </div>
        );
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          <Label className="text-xs">Scenes (= số ảnh)</Label>
          <Select
            value={String(config.scenes)}
            onValueChange={(v) => onUpdate({ scenes: parseInt(v, 10) })}
          >
            <SelectTrigger className="h-8 text-xs">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {[8, 12, 20, 30, 40, 50, 60, 80, 100].map((n) => (
                <SelectItem key={n} value={String(n)}>
                  {n} scenes (~{Math.round((n * config.duration) / 60)} phút)
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-1">
          <Label className="text-xs">Duration / Scene (chuyển cảnh)</Label>
          <Select
          ...
          {previewImage ? (
            <div className="relative group">
              ...
              <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
                <span className="text-[9px] bg-black/50 text-white/80 px-2 py-0.5 rounded-full">
                  AI Preview
                </span>
                <button
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider">
                Saved Previews ({savedPreviews.length})
              </span>
            </div>
    ...
    <div className="space-y-3">
      ...
      {sbResult?.scenes && sbResult.scenes.length > 0 ? (
        <div className="rounded-md border border-orange-500/20 bg-orange-500/5 p-3 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-semibold text-orange-400">
              📋 Prompts từ Storyboard ({sbResult.scenes.length} scenes)
            </span>
          </div>
            ...
            <div className="space-y-1.5">
              {sbResult.scenes.map((s) => (
                <div key={s.scene} className="flex items-start gap-1.5 text-[10px]">
                  <Badge variant="secondary" className="text-[9px] px-1 py-0 shrink-0 mt-0.5">
                    S{s.scene}
                  </Badge>
        ...
        <div className="rounded-md border border-orange-500/20 bg-orange-500/5 px-3 py-2">
          <p className="text-[10px] text-muted-foreground">
            💡 Chạy Step 2 (Storyboard) trước để tạo visual prompts cho từng scene. Image Gen sẽ tự
            động sử dụng các prompts đó.
          </p>
        </div>
      )}
    </div>
...
function VoiceoverConfig({
  ...
  const scriptSource = (() => {
    const result = (activeRun as { result?: { files?: Record<string, unknown> } } | undefined)
      ?.result;
    if (!result?.files) return null;
  ...
  const handleOptimizeForTTS = async () => {
    ...
    try {
      ...
      const scriptText =
        scriptSource.text.length > 8000 ? scriptSource.text.substring(0, 8000) : scriptSource.text;
  ...
  useEffect(() => {
    ...
    const generatePreview = async (): Promise<Blob> => {
      if (isGem) {
        ...
        const res = await fetch(
          `https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(config.voice)}`,
          {
            method: 'POST',
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (config.engine === 'edge-tts') {
        ...
        blob = await res.blob();
      } else if (config.engine === 'fish-speech') {
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1">
        ...
        <Select
          ...
          onValueChange={(v) => onUpdate({ speed: parseFloat(v) })}
        >
          <SelectTrigger className="h-8 text-xs">
            <SelectValue />
          </SelectTrigger>
      ...
      <div className="space-y-2">
        ...
        {previewError && <p className="text-[10px] text-red-400">⚠️ {previewError}</p>}
      </div>
      ...
      {scriptSource ? (
        <div className="space-y-2">
          ...
          {optimizerError && <p className="text-[10px] text-red-400">⚠️ {optimizerError}</p>}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {config.cleanedScript ? (
                    <Badge
                      variant="outline"
                      className="text-[9px] border-green-500/50 text-green-400"
                    >
                      đã áp dụng
                    </Badge>
                  ) : (
                    <Badge
                      variant="outline"
                      className="text-[9px] border-yellow-500/50 text-yellow-400"
                    >
                      chưa áp dụng
                    </Badge>
                  )}
              ...
              {showOptimized && (
                <div className="px-3 pb-3 space-y-2 border-t border-green-500/20 pt-2">
                  ...
                  <p className="text-[9px] text-muted-foreground">
                    💡 Khi áp dụng, TTS sẽ đọc bản đã tối ưu thay vì script gốc. Xóa để dùng lại
                    script gốc.
                  </p>
                </div>
              )}
...
function AssemblyConfig({
  ...
  const imagesJson = files['images.json'] as
    | { images?: unknown[]; successCount?: number }
    | undefined;
  const voiceoverJson = files['voiceover.json'] as
    | {
        clips?: unknown[];
        successCount?: number;
        totalDuration?: number;
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-md border bg-muted/30 px-3 py-2.5 space-y-1.5">
        <p className="text-[11px] font-medium text-muted-foreground">
          📋 Yêu cầu đầu vào (tự động lấy từ các step trước):
        </p>
        <div className="grid grid-cols-2 gap-2">
          <div
            className={`flex items-center gap-1.5 text-[11px] ${hasImages ? 'text-green-400' : 'text-muted-foreground'}`}
          >
            {hasImages ? '✅' : '⬜'} Step 3 — Images{' '}
            {hasImages && (
              <span className="text-[9px] opacity-70">({imagesJson?.successCount} ảnh)</span>
            )}
          </div>
          <div
            className={`flex items-center gap-1.5 text-[11px] ${hasAudio ? 'text-green-400' : 'text-muted-foreground'}`}
          >
            {hasAudio ? '✅' : '⬜'} Step 4 — Audio{' '}
            {hasAudio && (
              <span className="text-[9px] opacity-70">
                ({voiceoverJson?.successCount} clips, ~{voiceoverJson?.totalDuration}s)
              </span>
            )}
          </div>
        </div>
        {!hasImages && !hasAudio && (
          <p className="text-[10px] text-yellow-400 mt-1">
            ⚠️ Chạy Step 3 + Step 4 trước, sau đó bật toggle và chạy Step 5.
          </p>
        )}
        {hasImages && !hasAudio && (
          <p className="text-[10px] text-yellow-400 mt-1">
            ⚠️ Cần thêm audio — chạy Step 4 (Voiceover) trước.
          </p>
        )}
        {!hasImages && hasAudio && (
          <p className="text-[10px] text-yellow-400 mt-1">
            ⚠️ Cần thêm images — chạy Step 3 (Image Gen) trước.
          </p>
        )}
        {hasImages && hasAudio && (
          <p className="text-[10px] text-green-400 mt-1">
            ✅ Sẵn sàng! Bật toggle và nhấn "Run Video Assembly Only" để ghép video.
          </p>
        )}
      </div>
        ...
        <div className="space-y-3 pl-2 border-l-2 border-muted">
          <div className="space-y-1">
            ...
            <p className="text-[9px] text-muted-foreground/60">
              Thêm thời gian hiển thị ảnh trước/sau audio (pause effect)
            </p>
          </div>
          ...
          <div className="space-y-1">
            ...
            />
            <p className="text-[9px] text-muted-foreground/60">
              URL ảnh PNG trong suốt, sẽ hiện góc dưới phải video
            </p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-md border bg-muted/20 px-3 py-2 space-y-1">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {config.scenePadding > 0 && (
            <Badge variant="secondary" className="text-[9px]">
              ⏸ pad {config.scenePadding}s
            </Badge>
          )}
          {config.watermarkUrl && (
            <Badge variant="secondary" className="text-[9px]">
