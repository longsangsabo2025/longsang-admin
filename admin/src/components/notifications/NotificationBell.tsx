...
export function NotificationBell() {
  ...
  const { isRunning, upcomingCount, start, stop, requestPermission } =
    useScheduledPostNotifications(true);
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">{isRunning ? 'Bật' : 'Tắt'}</span>
              <Switch checked={isRunning} onCheckedChange={handleToggleNotifications} />
            </div>
          </div>
            ...
            <ScrollArea className="h-64">
              <div className="space-y-2">
                {upcomingPosts.map((post) => (
                  ...
                  >
                    <img src={post.image_url} alt="" className="h-12 w-12 object-cover rounded" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm truncate">{post.caption.substring(0, 40)}...</p>
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-xs">{post.platforms.join(', ')}</span>
                      </div>
                    </div>
                  </div>
                ))}
