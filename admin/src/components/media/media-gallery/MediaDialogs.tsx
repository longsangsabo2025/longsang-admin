...
import { formatDate, formatSize, typeConfig } from './utils';
  ...
  return (
    <>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) =>
          !open && setDeleteDialog({ open: false, item: null, permanent: false, bulk: false })
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
                  ...
                  <div className="mt-2 max-h-32 overflow-y-auto text-sm bg-muted/50 rounded-lg p-2">
                    ...
                    {selectedItems.length > 10 && (
                      <div className="text-muted-foreground">
                        ... và {selectedItems.length - 10} files khác
                      </div>
                    )}
                  </div>
                </>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setRenameDialog({ open: false, item: null, newName: '' })}
      >
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Nhập tên mới cho file</p>
              </div>
            </div>
            <Input
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                onClick={onRename}
                disabled={operationLoading || !renameDialog.newName.trim()}
              >
                {operationLoading ? (
                  <Loader2 className="h-4 w-4 animate-spin mr-2" />
                ) : (
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setCreateFolderDialog({ open: false, name: '' })}
      >
        <DialogContent className="sm:max-w-md">
          <VisuallyHidden>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold">Tạo Folder mới</h3>
                <p className="text-sm text-muted-foreground">
                  Folder sẽ được tạo trong thư mục hiện tại
                </p>
              </div>
            </div>
            <Input
              ...
              onChange={(e) =>
                setCreateFolderDialog({ ...createFolderDialog, name: e.target.value })
              }
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                ...
                disabled={operationLoading || !createFolderDialog.name.trim()}
              >
                {operationLoading ? (
                  <Loader2 className="h-4 w-4 animate-spin mr-2" />
                ) : (
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) setMoveDialog({ open: false, item: null, bulk: false });
        }}
      >
        <DialogContent className="sm:max-w-md">
          <VisuallyHidden>
            <DialogTitle>Di chuyển file vào Project</DialogTitle>
          </VisuallyHidden>
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold">
                  {moveDialog.bulk
                    ? `Di chuyển ${selectedItems.length} files`
                    : `Di chuyển "${moveDialog.item?.name}"`}
                </h3>
              ...
              <div className="p-2 space-y-1">
                {projects && projects.length > 0 ? (
                  projects.map((project) => (
                    <button
                      key={project.id}
                      onClick={() => onMove(project.folderId)}
                      disabled={operationLoading || project.folderId === projectFolderId}
                      className={cn(
                        'w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors text-left disabled:opacity-50',
                        project.folderId === projectFolderId
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {project.folderId === projectFolderId && (
                          <span className="text-xs text-muted-foreground">Project hiện tại</span>
                        )}
                      </div>
                      {project.folderId !== projectFolderId && (
            ...
            <div className="flex justify-end gap-2">
              <Button
                variant="outline"
                onClick={() => setMoveDialog({ open: false, item: null, bulk: false })}
              >
                Hủy
              </Button>
            </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setInfoDialog({ open: false, item: null })}
      >
        <DialogContent className="sm:max-w-md">
          <VisuallyHidden>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div
                className={cn(
                  ...
                  infoDialog.item ? typeConfig[infoDialog.item.type].bg : ''
                )}
              >
                {infoDialog.item &&
                  (() => {
                    const Icon = typeConfig[infoDialog.item.type].icon;
                    return (
                      <Icon className={cn('h-6 w-6', typeConfig[infoDialog.item.type].color)} />
                    );
                  })()}
              </div>
              <div className="min-w-0 flex-1">
                ...
                <Badge
                  className={cn(
                    'mt-1',
                    infoDialog.item ? typeConfig[infoDialog.item.type].bg : '',
                    infoDialog.item ? typeConfig[infoDialog.item.type].color : ''
                  )}
                >
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between py-2 border-b">
                ...
                <span className="font-medium">{infoDialog.item?.mimeType || 'N/A'}</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-muted-foreground">Kích thước</span>
                <span className="font-medium">{formatSize(infoDialog.item?.size) || 'N/A'}</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-muted-foreground">Ngày sửa đổi</span>
                <span className="font-medium">
                  {infoDialog.item?.modifiedTime ? formatDate(infoDialog.item.modifiedTime) : 'N/A'}
                </span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-muted-foreground">ID</span>
                <span className="font-mono text-xs">{infoDialog.item?.id}</span>
              </div>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                className="flex-1"
                onClick={() => {
                  if (infoDialog.item) onCopyUrl(infoDialog.item);
                }}
              >
              ...
              <Button
                variant="outline"
                className="flex-1"
                onClick={() => infoDialog.item && window.open(infoDialog.item.url, '_blank')}
              >
                <ExternalLink className="h-4 w-4 mr-2" /> Mở Drive
              </Button>
            </div>
          </div>
        </DialogContent>
