...
function useServiceHealth() {
  ...
  const check = useCallback(async () => {
    setServices((prev) =>
      prev.map((s) => ({ ...s, status: 'checking' as const, detail: 'Checking...' }))
    );
    ...
    // TTS Server
    try {
      const res = await globalThis.fetch('http://localhost:8100/health', {
      ...
      results.push({
        ...
        icon: Mic,
        status: 'offline',
        detail: 'Not reachable (8100)',
      });
    }

    // ComfyUI
    try {
      const res = await globalThis.fetch('http://localhost:8188/system_stats', {
        signal: AbortSignal.timeout(3000),
      });
      results.push({
        name: 'ComfyUI',
...
function CostTrackerSection({
  data,
  loading,
}: Readonly<{ data: PipelineCostData | null; loading: boolean }>) {
  if (loading) {
    return (
    ...
    <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ~{estimates.videosPerWeek}/week target
                </p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  ${estimates.cpmLow} – ${estimates.cpmHigh}
                </p>
      ...
      <Card className="bg-gradient-to-r from-green-500/5 to-emerald-500/5 border-green-500/20">
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            Based on {estimates.videosPerWeek * 4} videos/month × 500 views × ${estimates.cpmLow}–$
            {estimates.cpmHigh} CPM
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="text-center p-4 rounded-lg bg-background/60">
              ...
              <p className="text-xl font-bold text-green-500">
                {formatUSD(estimates.projectedMonthlyLow)}
              </p>
              <p className="text-xs text-muted-foreground">
                {formatVND(estimates.projectedMonthlyLow)}
              </p>
            </div>
            <div className="text-center p-4 rounded-lg bg-background/60">
              <p className="text-sm text-muted-foreground mb-1">Revenue (High)</p>
              <p className="text-xl font-bold text-green-500">
                {formatUSD(estimates.projectedMonthlyHigh)}
              </p>
              <p className="text-xs text-muted-foreground">
                {formatVND(estimates.projectedMonthlyHigh)}
              </p>
            </div>
            <div className="text-center p-4 rounded-lg bg-background/60">
      ...
      <div className="flex items-center gap-2 p-3 rounded-lg border border-yellow-500/30 bg-yellow-500/5 text-sm">
        ...
        <span className="text-muted-foreground">
          Revenue estimates use placeholder view counts. Connect YouTube Analytics API for real
          data.
        </span>
      </div>
    </div>
...
function AutomationHealthSection({
  services,
  onRefresh,
}: Readonly<{ services: ServiceHealth[]; onRefresh: () => void }>) {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <StatusIcon
                        className={`h-3.5 w-3.5 ${getHealthColor(service.status)} ${
