...
export function CacheStatsIndicator({
  ...
  const getColor = (rate: number) => {
    if (rate >= 50)
      return 'text-green-700 bg-green-50 border-green-200 dark:bg-green-950 dark:text-green-400';
    if (rate >= 30)
      return 'text-yellow-700 bg-yellow-50 border-yellow-200 dark:bg-yellow-950 dark:text-yellow-400';
    return 'text-orange-700 bg-orange-50 border-orange-200 dark:bg-orange-950 dark:text-orange-400';
  };

  const indicator = (
    <Badge variant="outline" className={cn('text-xs gap-1.5 font-medium', getColor(hitRateNum))}>
        ...
        <Tooltip>
          ...
          <TooltipContent>
            <div className="space-y-1 text-xs">
              ...
              <p>
                <strong>Hits:</strong> {stats.hits} | <strong>Misses:</strong> {stats.misses}
              </p>
              <p>
                <strong>Cached:</strong> {stats.size}/100 entries
              </p>
