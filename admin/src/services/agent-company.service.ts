...
async function queryRegistry(filters?: {
  ...
  let query = supabase.from('agent_registry').select('*').order('department').order('name');
...
export const agentCompanyApi = {
  ...
  dashboard: {
    ...
    async get(): Promise<CompanyDashboard> {
      const res = await callEdgeFunction<{ success: boolean; company: CompanyDashboard }>(
        'dashboard'
      );
      return res.company;
    },
  ...
  executions: {
    ...
    async listRunnerAgents(): Promise<{
      success: boolean;
      agents: Array<{
        codename: string;
        name: string;
