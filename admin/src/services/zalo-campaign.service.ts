...
class ZaloCampaignService {
  ...
  async getCampaign(id: string): Promise<Campaign | null> {
    const { data, error } = await supabase.from('zalo_campaigns').select('*').eq('id', id).single();
  ...
  async updateCampaignStatus(id: string, status: Campaign['status']): Promise<boolean> {
    ...
    const { error } = await supabase.from('zalo_campaigns').update(updates).eq('id', id);
  ...
  async getCampaignStats(campaignId: string): Promise<CampaignStats> {
    ...
    const delivered = targets.filter((t) =>
      ['delivered', 'opened', 'clicked', 'converted'].includes(t.status)
    ).length;
    const opened = targets.filter((t) =>
      ['opened', 'clicked', 'converted'].includes(t.status)
    ).length;
  ...
  async getVouchers(campaignId?: string): Promise<Voucher[]> {
    let query = supabase.from('zalo_vouchers').select('*').eq('oa_id', this.oaId);
  ...
  async updateCustomerVoucherStatus(
    ...
    const { error } = await supabase.from('zalo_customer_vouchers').update(updates).eq('id', id);
  ...
  async addTargetsToCampaign(campaignId: string, segment: TargetSegment): Promise<number> {
    ...
    const { error } = await supabase.from('zalo_campaign_targets').insert(targets);
