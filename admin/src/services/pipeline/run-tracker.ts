...
/** Mark a run as completed, then merge any duplicate topic entries */
export function completeRun(run: GenerationRun, result: GenerationRun['result']) {
  ...
  run.hasResult = true;
  persistUpdate(run).catch(() => {});
  // Auto-merge duplicates for the same topic
  mergeRelatedRuns();
}
...
export function startProgressTracker(
  ...
  const timer = setInterval(() => {
    ...
    run.logs.push({
      t: Date.now(),
      level: 'info',
      msg: `[${phase.pct}%] ${phase.msg} (${elapsed}s)`,
      step,
    });
    phaseIdx++;
  }, interval);
 ...
 *  When topic is provided, only matches runs with the same topic — prevents cross-topic data leaks. */
export function findLatestRunWithFile(
  fileName: string,
  channelId?: string | null,
  scopeRunId?: string,
  topic?: string | null
  ...
  for (const run of clientRuns.values()) {
    ...
    if (run.status === 'failed') continue; // Skip failed runs — data may be corrupt
    if (!run.result?.files?.[fileName]) continue;
    // Topic isolation: only match runs with the same topic
    if (topic) {
      const runTopic = run.input?.topic || run.input?.transcript;
      if (runTopic !== topic) continue;
    }
...
/** Sweep all in-memory runs and fix any that are orphaned */
export function fixAllOrphanedRuns(): number {
  let fixed = 0;
  for (const run of clientRuns.values()) {
    if (run.status === 'running') {
      const before = run.status;
      fixOrphanedRun(run);
      if (run.status !== before) fixed++;
    }
  }
  return fixed;
}
...
/**
 * Merge runs with the same topic into a single entry.
 ...
 */
export function mergeRelatedRuns(runs?: GenerationRun[]): void {
  const targets = runs || [...clientRuns.values()];
  ...
  for (const [, group] of groups) {
    ...
    for (let i = 1; i < group.length; i++) {
      ...
      // Merge steps
      primary.pipelineSteps = [
        ...new Set([...(primary.pipelineSteps || []), ...(other.pipelineSteps || [])]),
      ];
      primary.completedSteps = [
        ...new Set([...(primary.completedSteps || []), ...(other.completedSteps || [])]),
      ];
