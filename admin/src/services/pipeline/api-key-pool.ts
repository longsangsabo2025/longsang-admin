export type EngineType = 'gemini' | 'openai' | 'elevenlabs' | 'google-tts';
...
// In-memory cache to avoid localStorage thrashing during rapid key rotation
let _poolCache: PoolEntry[] | null = null;
let _pinsCache: Record<string, string> | null = null;

function loadPins(): Record<string, string> {
  if (_pinsCache) return _pinsCache;
  try {
    _pinsCache = JSON.parse(localStorage.getItem(PINNED_KEY) || '{}');
  } catch {
    _pinsCache = {};
  }
  return _pinsCache!;
...
function load(): PoolEntry[] {
  ...
  try {
    _poolCache = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
  } catch {
    _poolCache = [];
  }
  return _poolCache!;
...
export async function hydrateFromDb(): Promise<void> {
  ...
  _poolCache = merged; // Update in-memory cache
  notify();
}
