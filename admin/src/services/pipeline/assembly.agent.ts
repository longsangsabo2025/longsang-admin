...
function getPanZoomParams(panZoom: string, progress: number): { scale: number; offsetX: number } {
  switch (panZoom) {
    case 'zoom-in':
      return { scale: 1 + progress * 0.12, offsetX: 0 };
    case 'zoom-out':
      return { scale: 1.12 - progress * 0.12, offsetX: 0 };
    case 'pan-left':
      return { scale: 1.08, offsetX: progress * -60 };
    case 'pan-right':
      return { scale: 1.08, offsetX: progress * 60 };
    default:
...
function renderFrame(
  ...
  const tProgress = isInTransition ? 1 - (scene.duration - sceneElapsed) / transitionDur : 0;
...
export async function runAssembly(runId: string, req: GenerateRequest): Promise<void> {
  ...
  if (!imagesJson?.images?.length) {
    const imgRun = findLatestRunWithFile('images.json', req.channelId, runId, runTopic);
    imagesJson = imgRun?.result?.files?.['images.json'] as { images?: SceneImage[] } | undefined;
  }
  if (!imagesJson?.images?.length) {
    failRun(
      run,
      'Cần có images từ Step 3 (Image Generation) trước! Chạy Step 3 trước khi assembly.'
    );
    return;
  }
  ...
  if (!voiceoverJson?.clips?.length) {
    const voRun = findLatestRunWithFile('voiceover.json', req.channelId, runId, runTopic);
    voiceoverJson = voRun?.result?.files?.['voiceover.json'] as VoiceoverJson | undefined;
  }
  if (!voiceoverJson?.clips?.length) {
    failRun(
      run,
      'Cần có voiceover từ Step 4 (Voiceover/TTS) trước! Chạy Step 4 trước khi assembly.'
    );
    return;
  }
  ...
  if (assemblyConfig.textOverlay) {
    storyboardJson = run.result?.files?.['storyboard.json'] as
  ...
  try {
    ...
    run.logs.push({
      t: Date.now(),
      level: 'info',
      msg: `📥 Loading ${totalScenes} images...`,
      step: 'assembly',
    });
    ...
    if (imgFailCount > 0) {
      run.logs.push({
        t: Date.now(),
        level: 'warn',
        msg: `⚠️ ${imgFailCount}/${totalScenes} images failed to load`,
        step: 'assembly',
      });
    }
    ...
    if (isSingleNarration && fullNarrationUrl) {
      run.logs.push({
        t: Date.now(),
        level: 'info',
        msg: '🔊 Loading single narration audio...',
        step: 'assembly',
      });
      ...
      run.logs.push({
        t: Date.now(),
        level: 'info',
        msg: `🔊 Single narration loaded: ${singleNarrationBuffer.duration.toFixed(1)}s`,
        step: 'assembly',
      });
    } else {
      run.logs.push({
        t: Date.now(),
        level: 'info',
        msg: `🔊 Loading ${clips.length} audio clips...`,
        step: 'assembly',
      });
        ...
        if (!uniqueUrls.has(clip.url)) {
          try {
            ...
            run.logs.push({
              t: Date.now(),
              level: 'warn',
              msg: `⚠️ Failed to load audio for scene ${clip.scene}`,
              step: 'assembly',
            });
          }
        }
      ...
      if (audioFailCount > 0) {
        run.logs.push({
          t: Date.now(),
          level: 'warn',
          msg: `⚠️ ${audioFailCount}/${clips.length} audio clips failed`,
          step: 'assembly',
        });
      }
    }
    ...
    if (assemblyConfig.watermarkUrl) {
      try {
        ...
        run.logs.push({
          t: Date.now(),
          level: 'warn',
          msg: '⚠️ Watermark failed to load, skipping',
          step: 'assembly',
        });
      }
    }
    ...
    const sceneNumbers = [
      ...new Set([...images.map((i) => i.scene), ...clips.map((c) => c.scene)]),
    ].sort((a, b) => a - b);
    ...
    if (isSingleNarration && singleNarrationBuffer) {
      ...
      for (const sceneNum of sceneNumbers) {
        ...
        const clipData = clips.find((c) => c.scene === sceneNum);
        const baseDuration = audioBuf
          ? audioBuf.duration
          : clipData?.duration || DEFAULT_SCENE_DURATION;
    ...
    const totalDuration =
      isSingleNarration && singleNarrationBuffer
    ...
    run.logs.push({
      t: Date.now(),
      level: 'info',
      msg: `⏱️ Total video duration: ${totalDuration.toFixed(1)}s (${timeline.length} scenes)`,
      step: 'assembly',
    });
    ...
    if (isSingleNarration && singleNarrationBuffer) {
      ...
      run.logs.push({
        t: Date.now(),
        level: 'info',
        msg: '🔊 Single narration audio scheduled (1 source)',
        step: 'assembly',
      });
    } else {
      ...
      run.logs.push({
        t: Date.now(),
        level: 'info',
        msg: `🔊 ${scheduledSources.length} audio sources scheduled`,
        step: 'assembly',
      });
    }
    ...
    run.logs.push({
      t: Date.now(),
      level: 'info',
      msg: `🎬 Real-time rendering: ${totalDuration.toFixed(1)}s at ${width}x${height} ${FPS}fps...`,
      step: 'assembly',
    });
    ...
    run.logs.push({
      t: Date.now(),
      level: 'info',
      msg: `📦 Video encoded: ${(videoBlob.size / 1024 / 1024).toFixed(1)} MB`,
      step: 'assembly',
    });
    ...
    run.logs.push({
      t: Date.now(),
      level: 'info',
      msg: '💾 Uploading video to storage...',
      step: 'assembly',
    });
