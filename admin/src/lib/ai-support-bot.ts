...
function getOpenAI() {
  if (!_openai) {
    ...
    if (!apiKey) throw new Error('OpenAI API key not configured. Set VITE_OPENAI_API_KEY.');
...
If you don't know the answer, admit it and suggest contacting human support.`,
  ...
  async sendMessage(message: string): Promise<string> {
    ...
    await getOpenAI().beta.threads.messages.create(this.threadId!, {
      role: 'user',
      content: message,
    });

    // Run assistant
    const run = await getOpenAI().beta.threads.runs.create(this.threadId!, {
      assistant_id: this.assistantId,
    });

    // Wait for completion
    let runStatus = await getOpenAI().beta.threads.runs.retrieve(this.threadId!, run.id);

    while (runStatus.status !== 'completed') {
      await new Promise((resolve) => setTimeout(resolve, 1000));
      runStatus = await getOpenAI().beta.threads.runs.retrieve(this.threadId!, run.id);
    ...
    // Get messages
    const messages = await getOpenAI().beta.threads.messages.list(this.threadId!);
  ...
  async streamMessage(message: string, onChunk: (text: string) => void): Promise<void> {
    ...
    const stream = await getOpenAI().beta.threads.runs.stream(this.threadId!, {
      assistant_id: this.assistantId,
    });
