...
export const googleDriveAPI = {
  ...
  async moveBatch(items: Array<{ fileId: string; destinationId: string }>): Promise<{
  ...
  async renameFile(
    fileId: string,
    newName: string
  ): Promise<{ success: boolean; file: { id: string; name: string } }> {
    try {
  ...
  ): Promise<{ success: boolean; fileId: string; action: string }> {
    try {
      const url = permanent
