...
export class VectorStore {
  ...
  static async generateEmbedding(text: string): Promise<number[]> {
    try {
      const response = await getOpenAI().embeddings.create({
        model: VectorStore.EMBEDDING_MODEL,
        input: text,
        dimensions: VectorStore.EMBEDDING_DIMENSIONS,
      });
  ...
  static async storeDocument(doc: Document): Promise<string> {
    try {
      ...
      const embedding = doc.embedding || (await VectorStore.generateEmbedding(doc.content));
  ...
  static async answerWithRAG(
    ...
    try {
      ...
      const sources = await VectorStore.semanticSearch(question, {
        limit: 3,
        threshold: 0.75,
      });
  ...
  static async batchImport(documents: Document[]): Promise<string[]> {
    ...
    for (const doc of documents) {
      try {
        const id = await VectorStore.storeDocument(doc);
        ids.push(id);
      } catch (error) {
  ...
  static async updateDocument(id: string, updates: Partial<Document>): Promise<void> {
    try {
      ...
      if (updates.content) {
        ...
        updateData.embedding = await VectorStore.generateEmbedding(updates.content);
      }
