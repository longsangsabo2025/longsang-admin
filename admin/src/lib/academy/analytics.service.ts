...
export class AnalyticsService {
  ...
  static async getUserAnalytics(userId: string, courseId: string) {
    try {
      ...
      if (!data) {
        return await AnalyticsService.initializeAnalytics(userId, courseId);
      }
  ...
  static async getStudentProgressReport(userId: string, courseId: string) {
    try {
      const analytics = await AnalyticsService.getUserAnalytics(userId, courseId);
  ...
  static async getClassPerformanceReport(courseId: string) {
    try {
      const allAnalytics = await AnalyticsService.getCourseAnalytics(courseId);

      const students = allAnalytics.map((a) => ({
        ...a,
        engagement_score: AnalyticsService.calculateEngagementScore(a),
      }));
