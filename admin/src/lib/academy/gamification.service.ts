...
export class GamificationService {
  ...
  static async awardXP(
    ...
    try {
      ...
      await GamificationService.checkBadgeUnlocks(user.id);
  ...
  static async updateStreak(minutesLearned: number, lessonsCompleted: number): Promise<void> {
    try {
      ...
      if (streakCount === 3) {
        await GamificationService.awardXP('streak_3');
      } else if (streakCount === 7) {
        await GamificationService.awardXP('streak_7');
      } else if (streakCount === 30) {
        await GamificationService.awardXP('streak_30');
      }
