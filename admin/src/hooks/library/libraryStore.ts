...
import { MediaItem, ProductItem, ProductStatus, WorkspaceItem } from './types';
...
export const productsStore = {
  ...
  getStats: () => {
    ...
    items.forEach((item) => {
      if (item.status && Object.hasOwn(result, item.status)) {
        result[item.status]++;
      }
    });
...
async function fetchWorkspaceFromDrive(): Promise<void> {
  ...
  try {
    ...
    if (driveItems.length > 0) {
      ...
      console.log(
        `[Library] ✅ Workspace synced: ${driveItems.length} from Drive, ${localOnly.length} local`
      );
    }
...
async function fetchProductsFromDrive(): Promise<void> {
  ...
  try {
    ...
    if (driveItems.length > 0) {
      ...
      console.log(
        `[Library] ✅ Products synced: ${driveItems.length} from Drive, ${localOnly.length} local`
      );
    }
...
export const libraryActions = {
  ...
  addToProducts: async (
    ...
    // 2. Upload to Google Drive in background
    if (uploadToDrive && item.url) {
      uploadToGoogleDrive(item.url, item.name || `AI_Image_${Date.now()}.png`, 'products', status, {
        prompt: (item as any).prompt || '',
      }).then((result) => {
        if (result.success && result.file) {
          // Update item with Drive info
...
export { PRODUCTS_KEY, WORKSPACE_KEY };
