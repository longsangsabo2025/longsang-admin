...
import { ACTIVITY_ICONS, ActivityAction, ActivityLogEntry } from './types';
...
export function useActivityLog() {
  ...
  const logAction = useCallback(
    async (action: ActivityAction, description: string, count?: number) => {
  ...
  const clearAll = useCallback(async () => {
    ...
    try {
      ...
      await supabase.from('library_activity_log').delete().eq('user_id', session.session.user.id);
    } catch (error) {
