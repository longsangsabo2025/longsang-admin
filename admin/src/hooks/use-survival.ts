...
import type { DailyPlan, SurvivalMetrics, SurvivalTask } from '@/types/survival.types';
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<SurvivalMetrics> => {
      ...
      return {
        ...
        dailyTarget: Math.ceil(
          (settings.targetAmount - settings.currentAmount) / settings.daysRemaining
        ),
        currency: 'USD',
...
export function useCreateTask() {
  ...
  return useMutation({
    mutationFn: async (input: Partial<SurvivalTask>) => {
      ...
      // Calculate ICE score
      const ice_score = calculateICE(input.impact ?? 5, input.confidence ?? 5, input.ease ?? 5);
...
export function useDeleteTask() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      // Try Supabase
      try {
        const { error } = await supabase.from('survival_tasks').delete().eq('id', id);
...
function calculateFocusScore(tasks: SurvivalTask[]): number {
  ...
  const totalToday = tasks.filter(
    (t) => t.created_at.startsWith(today) || (t.status === 'pending' && t.quadrant === 'do_now')
  );
