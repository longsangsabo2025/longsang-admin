#!/usr/bin/env node
/**
 * 🚀 AUTO-INJECT ERROR REPORTER
 * Tự động cài đặt error tracking vào TẤT CẢ các project
 * 
 * Chạy: node scripts/auto-inject-error-reporter.cjs
 */
...
function injectErrorReporter(project) {
  ...
  if (!fs.existsSync(projectPath)) {
    console.log(`⏭️  Skip: ${project.name} (không tồn tại)`);
  ...
  console.log(`✅ Created: ${project.name}/src/lib/error-reporter.ts`);

  // 2. Find and modify entry file
  const entryFile = findEntryFile(projectPath);
  if (!entryFile) {
    console.log(`⚠️  ${project.name}: Không tìm thấy entry file`);
  ...
  if (entryContent.includes('error-reporter')) {
    console.log(`✅ ${project.name}: Đã có import error-reporter`);
  ...
  console.log(`✅ Injected import into: ${project.name}/${entryFile.relative}`);
...
// MAIN
// ============================================
console.log('🚀 AUTO-INJECT ERROR REPORTER');
...
console.log('='.repeat(50));
console.log('📊 KẾT QUẢ:');
console.log(`✅ Thành công: ${results.success.length} (${results.success.join(', ')})`);
console.log(`⏭️  Bỏ qua: ${results.skipped.length} (${results.skipped.join(', ')})`);
console.log(`❌ Thất bại: ${results.failed.length} (${results.failed.join(', ')})`);
console.log('='.repeat(50));
console.log('');
console.log('🎯 NEXT STEPS:');
console.log('1. Commit & push các project đã inject');
console.log('2. Deploy lại các project');
console.log('3. Khi có lỗi thật → Tự động gửi về longsang-admin → Tự động tạo Issue');
