...
export function SupabaseTest() {
  ...
  const runTests = async () => {
    ...
    if (!supabaseUrl || !supabaseKey) {
      updateTest('Environment', 'error', 'Missing Supabase credentials', 
        `URL: ${supabaseUrl ? '✓' : '✗'}, Key: ${supabaseKey ? '✓' : '✗'}`);
    } else {
      updateTest('Environment', 'success', 'Configuration loaded', 
    ...
    try {
      ...
      if (response.ok || response.status === 404) {
        updateTest('Network', 'success', 'Network connection OK', 
          `Status: ${response.status}`);
      } else {
        updateTest('Network', 'error', 'Unexpected response', 
          `Status: ${response.status}`);
      }
    } catch (error: any) {
      updateTest('Network', 'error', 'Network error', 
    ...
    try {
      if (supabase) {
        updateTest('Client', 'success', 'Client initialized', 
          'Supabase client is ready');
      } else {
        updateTest('Client', 'error', 'Client not initialized', 
    ...
    try {
      ...
      if (error) {
        ...
        updateTest('Database', 'success', 'Query successful', 
    ...
    try {
      ...
      if (error) {
        ...
        updateTest('Auth', 'success', 'Authenticated', 
          `User: ${session.user.email}`);
      } else {
        updateTest('Auth', 'success', 'Not authenticated', 
    ...
    <div className="container max-w-4xl mx-auto py-8 space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          Diagnose connection issues with Supabase backend
        </p>
      </div>
      ...
      {tests.length > 0 && (
        <Alert variant={allSuccess ? "default" : hasErrors ? "destructive" : "default"}>
          ...
          <AlertTitle>
            {allSuccess ? '✅ All tests passed!' : hasErrors ? '❌ Connection issues detected' : 'Testing...'}
          </AlertTitle>
          <AlertDescription>
            {allSuccess 
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          <div className="flex justify-between items-center">
            ...
            <code className="text-xs bg-muted px-2 py-1 rounded">
              {import.meta.env.VITE_SUPABASE_ANON_KEY ? '***' + import.meta.env.VITE_SUPABASE_ANON_KEY.slice(-8) : 'Not set'}
            </code>
          </div>
          <div className="flex justify-between items-center">
          ...
          <CardContent className="space-y-3">
            <div className="space-y-2">
              ...
              <ul className="text-sm space-y-1 list-disc list-inside text-muted-foreground">
                ...
                <li>Try accessing the URL in browser: <code className="text-xs bg-muted px-1">{import.meta.env.VITE_SUPABASE_URL}</code></li>
            ...
            <div className="space-y-2">
              ...
              <ul className="text-sm space-y-1 list-disc list-inside text-muted-foreground">
                ...
                <li>Run database migrations: <code className="text-xs bg-muted px-1">supabase db reset</code></li>
              </ul>
            </div>
            <div className="space-y-2">
              <h4 className="font-semibold text-sm">💡 Development Mode</h4>
              <ul className="text-sm space-y-1 list-disc list-inside text-muted-foreground">
                <li>Consider using local Supabase: <code className="text-xs bg-muted px-1">supabase start</code></li>
