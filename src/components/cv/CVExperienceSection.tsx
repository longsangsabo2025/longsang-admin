        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              ...
              >
                {/* Content Card */}
                <div className={`lg:w-[calc(50%-3rem)] w-full ${exp.side === "left" ? "lg:text-right" : "lg:text-left"}`}>
                  <div
                    className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-card hover:shadow-elevated hover:border-primary/50 transition-all hover-lift group"
                  >
                    {/* Company & Logo */}
                    <div className={`flex items-center gap-4 mb-4 ${exp.side === "left" ? "lg:flex-row-reverse lg:justify-end" : "lg:justify-start"} justify-start`}>
                      <div className={`w-16 h-16 rounded-xl ${exp.iconBg} border-2 border-${exp.iconColor.replace('text-', '')}/30 flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all`}>
                    ...
                    {/* Period */}
                    <div className={`inline-flex items-center gap-2 px-4 py-2 bg-primary/10 border border-primary/20 rounded-full mb-4 ${exp.side === "left" ? "lg:float-right lg:ml-4" : ""}`}>
                      <Calendar className="w-4 h-4 text-primary" />
                      <span className="text-sm font-medium text-primary">{exp.period}</span>
                    </div>

                    {/* Description */}
                    <ul className={`space-y-2 mb-6 clear-both ${exp.side === "left" ? "lg:text-right" : "lg:text-left"} text-left`}>
                      {exp.description.map((item, i) => (
                        <li key={i} className="text-foreground-secondary flex items-start gap-2">
                          <span className={`text-primary mt-1 ${exp.side === "left" ? "lg:order-2" : ""}`}>•</span>
                    ...
                    {/* Skills Tags */}
                    <div className={`flex flex-wrap gap-2 ${exp.side === "left" ? "lg:justify-end" : "lg:justify-start"} justify-start`}>
