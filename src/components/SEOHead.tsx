...
import { generateOrganizationSchema, generateArticleSchema, injectSchemaMarkup } from '../utils/schema-markup';
...
export const SEOHead: React.FC<SEOProps> = ({
  ...
  const finalCanonical = canonical || (typeof window !== 'undefined' ? window.location.href : baseUrl);
...
export const TournamentSEO: React.FC<TournamentSEOProps> = ({ tournament }) => {
  ...
  const description = tournament.description || `Join ${tournament.name} tournament at SABO ARENA. Experience competitive gaming at its finest.`;
  ...
  const eventSchema = {
    ...
    image: tournament.image ? `https://longsang.org/arena${tournament.image}` : 'https://longsang.org/arena/og-image-tournament.jpg',
    organizer: {
      '@type': 'Organization',
      name: 'SABO ARENA',
