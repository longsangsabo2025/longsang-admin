 ...
 */
export async function postToSocialMedia(request: SocialPostRequest): Promise<SocialPostResponse> {
  // Check if we're in mock mode
  const hasAnyToken = LINKEDIN_ACCESS_TOKEN || TWITTER_ACCESS_TOKEN || FACEBOOK_ACCESS_TOKEN;

  if (!hasAnyToken) {
    console.warn(
      `⚠️ No ${request.platform} API token configured. Post would be published in production.`
    );
